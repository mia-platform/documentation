...
The [Enhanced Project Workflow](/docs/12.x.x/development_suite/set-up-infrastructure/enhanced-project-workflow) is a BETA feature that aims to improve the developer experience when working with the Console, as well as paving the way for the adoption of previously unavailable opportunities, such as the [GitOps deployment strategy](/docs/12.x.x/development_suite/deploy/gitops-based/index).  
...
In fact, when using the Enhanced Project Workflow, configurations are generated and committed to the Git repository only at deployment time. Consequently, it becomes possible to choose if adopting a [pipeline-based deployment strategy](/docs/12.x.x/development_suite/deploy/pipeline-based/index) or a GitOps approach that facilitates the automated deployment and management of applications on Kubernetes clusters, making the deployment process more reliable, reproducible, and efficient, without the need to trigger any CI/CD pipelines.
...
To learn more about Listeners management, read the dedicated [documentation page](/docs/12.x.x/development_suite/api-console/api-design/listeners).
...
:::info
To learn more about `Import / Export`, please visit the dedicated [documentation page](/docs/12.x.x/fast_data/configuration/import_export).
:::
...
To learn more about how to configure Azure Key Vault as a secret manager on Console, visit the dedicated [documentation page](/docs/12.x.x/console/project-configuration/manage-environment-variables/manage-environment-variables-with-akv).
...
For further details on how to better manage your Company’s Marketplace through miactl, refer to the related documentation available [here](/docs/12.x.x/marketplace/add_to_marketplace/manage_marketplace_items#how-to-use-miactl-to-manage-the-marketplace), along with the list of new [miactl Marketplace commands](/docs/12.x.x/cli/miactl/30_commands#marketplace).

### Extension Providers - Orchestrator Generator

From Providers section in Company Overview it is possible to connect a new Provider type: the [**Extension API**](/docs/12.x.x/console/company-configuration/providers/extension-providers), useful to extend or replace some Console features.
...
To learn more about the capability of the Orchestrator Generator Extension and how to connect it to your Project, visit the [dedicated documentation](/docs/12.x.x/console/company-configuration/providers/extensions/orchestrator-generator/overview).

:::info
The Orchestrator Generator extension can only be used when the [Enhanced Project Workflow](/docs/12.x.x/development_suite/set-up-infrastructure/enhanced-project-workflow) is enabled.
:::
...
:::danger
There is a known bug regarding the management of _pr-update_ topic definitions in the Real-Time Updater: if the topic name is left empty inside the [projections detail page](/docs/12.x.x/fast_data/configuration/projections#pr-update-topic), the Real-Time Updater recognize the empty value as a real topic, causing the service to crash.

This issue has been solved in the v12.0.3 of the console. Please refer to the [dedicated release note](/docs/12.x.x/release-notes/v12.0.3) for further information.
:::
...
Attaching a Single View Trigger Generator will simplify the configuration of the service itself by automatically setting the same ER Schema config map used in the Single View Creator, and including a dedicated space where to configure the [Kafka Projection Updates](/docs/12.x.x/fast_data/configuration/config_maps/kafka_projection_updates) and the [Projection Changes Schema](/docs/12.x.x/fast_data/configuration/config_maps/projection_changes_schema) config map.
...
Any update here will be reflected in the Single View Creator service after saving the configuration. For more information, [read more on the dedicated documentation page](/docs/12.x.x/fast_data/configuration/single_view_creator/upsert_delete_strategies). 
...
For more information check out the service [documentation](/docs/12.x.x/fast_data/projection_storer).
...
For more information check out the service [changelog](/docs/12.x.x/runtime_suite/ingestion-storer/changelog).
...
To learn more about this feature, check out the dedicated [documentation page](/docs/12.x.x/runtime_suite/flow-manager-service/30_configuration#states-of-the-machine).
...
To learn more about this feature, check out the dedicated [documentation page](/docs/12.x.x/runtime_suite/flow-manager-service/30_configuration#command-hook).
...
To learn more about, check out the [documentation page](/docs/12.x.x/runtime_suite/authentication-service/10_overview).
...
To learn more about, check out the dedicated [documentation page](/docs/12.x.x/runtime_suite_applications/dev_portal/marketplace_management).
...
To learn more about, check out the dedicated [documentation page](/docs/12.x.x/runtime_suite/files-service/configuration).
...
For self-hosted installations, please head to the [self hosted upgrade guide](/docs/12.x.x/infrastructure/self-hosted/installation-chart/100_how-to-upgrade#v12---version-upgrades) or contact your Mia-Platform referent and upgrade to _Console Helm Chart_ `v11.0.3-2`.
