...
Envoy supports **external authorization** via the dedicated [HTTP filter](https://www.envoyproxy.io/docs/envoy/v1.21.0/configuration/http/http_filters/ext_authz_filter.html). We provide a compatible [authorization service](/runtime-components/plugins/authorization-service/10_overview.md) with its default Envoy filter, that will be added automatically to the filter chain when our external authorizer is picked from the Marketplace and deployed. However, you are free to deploy and configure your own in [advanced mode](/products/console/api-console/advanced-section/api-gateway-envoy/extensions.md#external-authorization).
...
Envoy supports **JWT authentication**, which can be combined with our **[Authorization Service**](/runtime-components/plugins/authorization-service/30_usage.md#trust-mia-platform-user-headers)** using the trusted user headers mode.
