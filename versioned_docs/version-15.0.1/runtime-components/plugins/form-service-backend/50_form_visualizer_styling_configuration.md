...
- **file**
  ...
  - *description*: contains the [information of a file](/runtime-components/plugins/files-service/configuration.mdx) uploaded using the `File Service`. The form visualizer will use the url contained in the `location` property of this object to load the required asset when needed.
...
You also need to expose a new endpoint using the same name defined in the configuration's **formStyleAssetsCrud** property following [this guide](/products/console/api-console/api-design/endpoints.md). A **formStyleAssetsCrud** value equal to `/form-style-assets` will require an endpoint with **Base path** equal to `/form-style-assets`.

:::info

Mia-Platform [Microfrontend Composer](/products/microfrontend-composer/back-kit/10_overview.md) can be used to manage the different style assets. In case you require supporting files, you will need to configure a [Files Service](/runtime-components/plugins/files-service/configuration.mdx) instance in your project and update Microfrontend Composer configuration to use it's [File Manager](/products/microfrontend-composer/back-kit/60_components/260_file_manager.md) client.
...
The CRUD collection described in the service [configuration](/runtime-components/plugins/form-service-backend/20_configuration.md#create-required-cruds) section must be updated with an additional property:
