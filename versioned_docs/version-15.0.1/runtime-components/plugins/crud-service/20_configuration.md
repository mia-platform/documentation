...
This service can be added to your project by visiting Mia-Platform [Marketplace](/runtime-components/overview_marketplace.md) and creating a new microservice from the **CRUD Service** plugin.
...
:::info
The CRUD Service supports custom CA certs. If you want to learn more about these certificates and how to configure them in your CRUD Service, visit [this page](/products/console/api-console/api-design/services.md#provide-a-ca-certificate-to-a-custom-service).
:::
...
However, you will find a link that will redirect you to **MongoDB CRUD** dedicated section where you can continue to configure your project [CRUDs](/products/console/api-console/api-design/crud_advanced.md). By doing so, you will automatically define the collections that will be handled by the service, which means that there is no need to add any configuration files.
...
| ENABLE_TRACING                  | Boolean | Optional | `false`                                   | Specifies if OpenTelemetry tracing should be enabled. It is possible to find more documentation [here](/runtime-components/libraries/lc39/main-entrypoint.md#opentelemetry-tracing-experimental)                 |
...
you should also include several specific Environment Variables, either you are using the [encryption with Google Platform Cloud](/runtime-components/plugins/crud-service/30_encryption_configuration.md#configure-csfle-with-the-google-cloud-platform-gcp)
or a [local key](/runtime-components/plugins/crud-service/30_encryption_configuration.md#configure-csfle-with-local-key).
