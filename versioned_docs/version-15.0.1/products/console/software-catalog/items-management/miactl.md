...
The center of our journey is a **Git repository** that will store the [manifests](/products/console/software-catalog/items-manifest/overview.md) of the Catalog items, alongside any additional resource they might need (e.g., items logos).
...
The folders hierarchical structure reflects that fact that each item has a **type**, at least one **version**, and, possibly, some **assets**. The JSON and YAML files inside the `versions` folders are the items [manifests](/products/console/software-catalog/items-manifest/overview.md).
