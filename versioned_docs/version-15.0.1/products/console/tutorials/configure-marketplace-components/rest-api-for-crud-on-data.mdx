...
:::info
If you have already done the [previous tutorial](/products/console/tutorials/configure-marketplace-components/microservice-hello-world.mdx), you can skip the rest of the section and directly start the tutorial.
:::
...
1. Have a properly configured [API Portal](/runtime-components/plugins/api-portal/10_overview.md) and [Swagger Aggregator](/runtime-components/plugins/swagger-aggregator/10_overview.md).
...
The first step is to create the [_CRUD Service_](/runtime-components/plugins/crud-service/10_overview_and_usage.md) plugin, its purpose is to abstract Data Collections allowing developers to expose CRUD APIs over the database in an easy, scalable, and secure way.
...
- in the _Indexes_ section, an [index](/products/console/api-console/api-design/crud_advanced.md#indexes) on the `_id` field and one on the `createdAt` field has been created
- in the _Internal Endpoints_, `/products` [endpoint](/products/console/api-console/api-design/crud_advanced.md) has been created with default `__STATE__` set as `PUBLIC`.
...
:::tip
You can import fields during the creation phase of a new CRUD Collection, by clicking on the "Import fields from file" button, as you can see in the [image](/products/console/tutorials/configure-marketplace-components/rest-api-for-crud-on-data.mdx#create-a-new-crud-collection).
:::
...
:::tip
If you don't know how to do it take a look at the [first tutorial](/products/console/tutorials/configure-marketplace-components/microservice-hello-world.mdx#expose-the-hello-world-microservice-api)
:::
...
Our configuration is ready. You can **save** and **deploy** it in a runtime environment! You don't know how to do it? Take a look at [Mia-Platform handbook](/products/console/handbooks/project-usage.md#deploy-changes)!
