...
The configuration of the service is handled by a JSON file whose location is defined by the `BFF_CONFIGURATION_FILEPATH`. When instantiating
 [Control Plane application](/runtime-components/applications/fast-data-control-plane/10_overview.md) from Marketplace, Fabric BFF service configuration is generated with
...
In order for Control Plane application to know which [Runtimes](/products/fast_data/runtime_management/overview.mdx#runtime) are available for monitoring, and therefore which ones can be included in
[Runtime Views](/products/fast_data/runtime_management/overview.mdx#runtime-view), the service needs to contact Mia-Platform Console and retrieve the list of Projects that should be accessible from this Control Plane instance.
...
| `/fast-data/*`        | REST      | *      | Other routes are forwarded towards Control Plane service.<br/>For more details please read [corresponding documentation](/products/fast_data/runtime_management/control_plane.mdx#endpoints) |
