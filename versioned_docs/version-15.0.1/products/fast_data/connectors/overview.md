...
- [MySQL](/products/fast_data/connectors/debezium_cdc.md#mysql)
- [Oracle DB](/products/fast_data/connectors/debezium_cdc.md#oracle-db)
- [PostgreSQL](/products/fast_data/connectors/debezium_cdc.md#postgresql)
- [IBM DB2](/products/fast_data/connectors/debezium_cdc.md#db2)
...
In general, any application that writes messages onto an event messaging platform adopting one of the formats [accepted by the Real-Time Updater](/products/fast_data/concepts/inputs_and_outputs.md#data-change-message) can act as a Connector.
In particular, it is possible to either write your own [custom Connector](#bring-your-own-cdc) tailored for your unique needs and requirements, or use one of the Connectors available in the Mia-Platform [Marketplace](/runtime-components/plugins/mia-platform-plugins.md).

Some connectors you'll find in the Marketplace:

- [CSV Connector](/runtime-components/applications/csv-connector/overview.md)
- [Debezium plugin Connectors](/products/fast_data/connectors/debezium_cdc.md#debezium-server-configuration)
...
Change Data Capture systems described above publish change events in one of the [formats supported by Fast Data](/products/fast_data/concepts/inputs_and_outputs.md#data-change-message). When another CDC system is employed it is important to either verify that such component can produce compatible messages or provide a [_custom message adapter_](/products/fast_data/configuration/realtime-updater/configuration-files.md#custom) to the Fast Data Real-Time Updater component. In this way Fast Data can easily process change events employing user-defined formats.
...
More details can be found in the dedicated Debezium Server configuration [page](/products/fast_data/connectors/debezium_cdc.md).
