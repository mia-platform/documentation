...
3. Log into the Console, go to the Design Section -> Advanced section and create the `auth0-client` configuration from scratch as an extension. To configure auth0-client, visit [its configuration page](/runtime-components/plugins/auth0-client/20_configuration.md). Configure also the env variables (`REDIS_HOSTS` is required).
