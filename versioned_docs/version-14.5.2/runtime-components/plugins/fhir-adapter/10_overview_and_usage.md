...
The FHIR Adapter has to work with a FHIR Server. In the Mia-Platform [Marketplace](/runtime-components/overview_marketplace.md) there is a ready to use server, called Mia FHIR Server, that can be associate with the FHIR Adapter. However, any server that implements the FHIR standard can couple with the FHIR Adapter.
...
For further info about translating query parameters for FHIR resources, please refer to the [Configuration section](/runtime-components/plugins/fhir-adapter/30_configuration.md#search-settings).
