...
This procedure is inspired by this [guide](/products/console/tutorials/configure-marketplace-components/auth-architecture/external-idp-internal-session.md)
...
- access to [Fast Data Control Plane](/runtime-components/applications/fast-data-control-plane/10_overview.md) application, which automatically
...
- an Identity Provider, which manages users' identities. The one employed throughout this guide is [Okta](https://www.okta.com/),
  although any other Identity Provider supported by the Mia-Platform [Authentication Service](/runtime-components/plugins/authentication-service/10_overview.md)
...
- [`api-gateway`](/runtime-components/plugins/envoy-api-gateway/overview.md), which is the entrypoint of your project requests
- [`authentication-service`](/runtime-components/plugins/authentication-service/10_overview.md), which is the service that interacts with your Identity Provider to verify your users identity
- [`crud-service`](/runtime-components/plugins/crud-service/10_overview_and_usage.md), which allows to query via HTTP requests the underlying
...
is a need for updating any specific detail, please look for more information at the dedicated [documentation page](/runtime-components/plugins/envoy-api-gateway/overview.md).
...
:::tip
In case you would prefer using another Identity Provider, please read the [authentication-service documentation](/runtime-components/plugins/authentication-service/20_configuration.mdx) to learn
...
:::caution
These instructions below assume that the tool employed for deploying Console projects is [`mlp`](/runtime-components/tools/mlp/10_overview.md).
...
In case more details are needed on this latter part, please head over to the `authentication-service` [documentation page](/runtime-components/plugins/authentication-service/30_usage.md#asymmetric-signing-algorithm-rsa256).
...
:::tip
In case it is needed to [embed Fast Data Control Plane solution within Mia-Platform Console](/products/fast_data/runtime_management/control_plane_frontend.mdx#embed-as-console-extension),
...
that is either interpolated from a [Project Secret Variable](/products/console/project-configuration/manage-environment-variables/index.md) or loaded from a K8s secret.
...
:::note
Before proceeding, be sure to have configured Control Plane endpoints accordingly to their [specific documentation](/products/fast_data/runtime_management/control_plane.mdx#endpoints).
:::
...
:::note
If you're using API Gateway with Nginx, please consider using [this approach for automatic redirects](/runtime-components/applications/secure-api-gateway/20_configuration.md#nginx).
:::
...
- [`authentication-service`](/runtime-components/plugins/authorization-service/10_overview.md), which is in charge of verifying whether an
...
on your ingress controller or an [API key](/products/console/api-console/api-design/api_key.md) to be verified by the Authorization Service.

Configuration of an API Key can be carried out directly in Console as explained [here](/products/console/api-console/api-design/api_key.md#create-a-new-api-key),
whereas in this [documentation page](/products/console/api-console/api-design/endpoints.md#manage-the-security-of-your-endpoints) is explained
...
:::tip
Upon securing gRPC endpoints with an API Key, please remember to [add the needed header](/products/fast_data/runtime_management/control_plane_operator.mdx#upstream)
...
This can be achieved thanks to the [Microfrontend Composer](/products/microfrontend-composer/what-is.md) tool, which allows crafting
