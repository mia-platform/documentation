...
:::caution Deprecated
The Single View Creator Template has been **deprecated** in favour of the [Single View Creator Plugin](/products/fast_data/configuration/single_view_creator/plugin.md), which supports both [Low Code and No Code features](/products/fast_data/no_code_overview.md).
We strongly suggest not to use the Single View Creator Template.
:::

Search in the [Marketplace](/runtime-components/overview_marketplace.md) for a `Single View Creator - Template` and create it.
...
First, the template uses the [Custom Plugin Lib](/runtime-components/libraries/custom-plugin-lib/apidoc.md) to instantiate a service.
...
`config` is an object whose fields represent the [Microservice environment variables](/products/console/api-console/api-design/services.md#environment-variable-configuration).
...
You can either use the [provided upsert and delete strategies](/products/fast_data/configuration/single_view_creator/upsert_delete_strategies.md) or to pass your custom functions directly to the `startCustom` method.
