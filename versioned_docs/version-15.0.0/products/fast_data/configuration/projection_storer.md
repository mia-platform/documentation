...
For an overview of which are Real-Time Updater's features, it is possible to read the introduction documentation [here](/products/fast_data/realtime_updater.md), whereas
[here](/products/fast_data/projection_storer.md) can be found a brief introduction to Projection Storer service.
:::

:::danger
Projection Storer plugin does not support the Fast Data [_standard_ architecture](/products/fast_data/concepts/architecture.md#standard-architecture).
However, it supports all the others architecture where [Projection Record Updates](/products/fast_data/concepts/inputs_and_outputs.md#projection-update-message) messages are emitted by the service as
triggers for the Fast Data downstream components. 

This means that, in order to use the Projection Storer, it will
be necessary to instantiate a [Single View Trigger Generator](/products/fast_data/single_view_trigger_generator.md) plugin to actually trigger the generation of Single Views.
:::
...
To evaluate data from external CDC, the Projections included in the System of Record must be attached to one or more [Real-Time Updater](/products/fast_data/realtime_updater.md) or Projection Storer. Services must be created in advance and they can be attached moving to the _Services_ tab of the selected System of Record.
...
:::tip
All the fields under configuration property support [secret resolution](/products/fast_data/configuration/secrets_resolution.md).
:::
...
values that this property can get, please refer to [cast functions](/products/fast_data/configuration/projection_storer.md#cast-functions-and-additional-cast-functions) section.
...
in [the dedicated section](/products/fast_data/runtime_management/workloads.mdx?workload=ps#projection-storer) of the configuration page.

Adding the Control Plane configuration to the service enables it to communicate with an instance of [Fast Data Control Plane](/products/fast_data/runtime_management/overview.mdx).
...
In this case, you just need to [send a `resume` command](/products/fast_data/runtime_management/control_plane_frontend.mdx#pause-and-resume)
...
:::caution
In order for the service to be aware of which Fast Data pipelines [_artifacts_](/products/fast_data/runtime_management/overview.mdx#artifact)
and [_execs_](/products/fast_data/runtime_management/overview.mdx#execution-step) it is managing, an additional
...
In the following section is explained how to migrate the configuration of an existing [Real-Time Updater](/products/fast_data/configuration/realtime-updater/realtime-updater.md) into the one
...
- supports both Real-Time Updater and its own custom message adapter definition. In fact, when the output of existing adapters is mapped internally to the newer one.  
  However, it is recommended to review existing implementation to reflect the newer return type, as explained in the [output paragraph](/products/fast_data/configuration/projection_storer.md#adapter-output).
...
Computing which Single View should be re-created given a specific change event is now a responsibility of the [Single View Trigger Generator](/products/fast_data/single_view_trigger_generator.md),
which should be configured accordingly. In this [page](/products/fast_data/configuration/single_view_trigger_generator.mdx) can be found an explanation on how to configure it.

In case the System of Record of your concern is currently adopting a Fast Data [_standard_ architecture](/products/fast_data/concepts/architecture.md#standard-architecture), which means
...
Projection Storer only supports Fast Data [_event-driven_ architectures](/products/fast_data/concepts/architecture.md#event-driven-architecture).
...
from _standard_ to _event-driven_ architecture is provided [here](/products/fast_data/single_view_trigger_generator.md#migration-guide-for-adopting-single-view-trigger-generator)
:::
