...
| MANUAL_STRATEGIES_FOLDER         | -        | Absolute path of custom strategy functions folder. These functions are used in `__fromFile__` annotations inside the [Kafka Projection Updates Configuration](/products/fast_data/configuration/config_maps/kafka_projection_updates.mdx)           | -             |
| TRIGGER_CUSTOM_FUNCTIONS_FOLDER  | -        | Absolute path of custom functions folder. These functions are used in `__fromFile__` annotations inside the [Projection Changes Schema Configuration](/products/fast_data/configuration/config_maps/projection_changes_schema.md#custom-functions). | -             |
| CONTROL_PLANE_CONFIG_PATH        | -        | Starting from `v3.3.0`, is possible to configure Runtime Management. More details [on the dedicated section](/products/fast_data/runtime_management/workloads.mdx?workload=svtg#single-view-trigger-generator)                                      | -             |
| CONTROL_PLANE_BINDINGS_PATH      | -        | Starting from `v3.3.0`, is possible to configure Runtime Management. More details [on the dedicated section](/products/fast_data/runtime_management/workloads.mdx?workload=svtg#single-view-trigger-generator)                                      | -             |
...
* create your Single View and attach a _Single View Creator Service_ to it [from the dedicated configuration page](/products/fast_data/configuration/single_view_creator/index.md#attach_to_single_view). This is necessary because the Single View Trigger Generator can work with only one Single View Creator Service.
...
Once the service has been attached, you can manage the content of the [_Projection Changes Schema config map_](/products/fast_data/configuration/config_maps/projection_changes_schema.md) and the [_Kafka Projection Updates config map_](/products/fast_data/configuration/config_maps/kafka_projection_updates.mdx).
...
The ER Schema config map contains the `erSchema.json` file which describes the relationships between each projection of the [System of Record](/products/fast_data/concepts/the_basics.md#system-of-record-sor).

Remember to copy/paste the mount path into the `ER_SCHEMA_FOLDER` environment variable so the service can read the file.
To know more on how to configure the file please go to the [ER Schema](/products/fast_data/configuration/config_maps/erSchema.md) page.
...
If you need more info on how to configure the `projectionChangesSchema.json` file, please refer to the [Projection Changes Schema](/products/fast_data/configuration/config_maps/projection_changes_schema.md) page.

### Kafka Projection Updates

The Kafka Projection Updates config map contains the `kafkaProjectionUpdates.json` file which defines the topics from where to consume the [Projection Updates](/products/fast_data/concepts/inputs_and_outputs.md#projection-update) and the strategy to apply to each message.
...
If you need more info on how to configure the `kafkaProjectionUpdates.json` file, please refer to the [Kafka Projection Updates](/products/fast_data/configuration/config_maps/kafka_projection_updates.mdx) page.
...
Starting from `v3.3.1`, the following fields can contain secrets that can be managed using the [Fast Data Secrets Resolution mechanism](/products/fast_data/configuration/secrets_resolution.md):
...
For the producers you can choose between two options: Kafka or MongoDB ([`sv-trigger` vs. `pc`](/products/fast_data/single_view_trigger_generator.md#sv-trigger-vs-pc)).
...
By specifying the environment variables `CONTROL_PLANE_CONFIG_PATH`, you enable the SVTG to receive and execute the commands from the [Runtime Management](/products/fast_data/runtime_management/overview.mdx).
...
You can read about the setup of the Single View Trigger Generator in [its dedicated section](/products/fast_data/runtime_management/workloads.mdx?workload=svtg#single-view-trigger-generator).
