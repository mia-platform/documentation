...
:::info
In the example described above, to be able to upload the additional metadata `key-1`, it is important to add a [caster file](/runtime_suite/files-service/configuration.mdx#caster-file) and to specify its path in the environment variable `ADDITIONAL_FUNCTION_CASTER_FILE_PATH`.
