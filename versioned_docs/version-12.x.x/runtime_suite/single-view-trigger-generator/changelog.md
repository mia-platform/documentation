...
For more details on how to configure the control plane, visit its [dedicated section](/fast_data/runtime_management.mdx#up-and-running)  
...
- kafka configuration is not passed through environment variables but through its own config map [`Input Output Config`](/fast_data/configuration/single_view_trigger_generator.md#input-output-config)
