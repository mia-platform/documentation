...
This job can be triggered by configuring a __Connection__ from [its section in the Data Catalog UI](/products/data_catalog/frontend/data_catalog_connections.mdx).
...
_Connection Cleanup_ is a set of procedures that are launched automatically when a user [deletes a connection from the Data Catalog UI](/products/data_catalog/frontend/data_catalog_connections.mdx#delete-connection).
...
_Fast Data Jobs Sync_ is a procedure that generates [Open Lineage Jobs](https://openlineage.io/docs/spec/facets/job-facets/) from existing [Fast Data pipelines of the Runtime Management](/products/fast_data/runtime_management/overview.mdx).
...
that is exposed under proper permissions as part of the [Secure Access microfrontend](/products/data_catalog/secure_access.mdx#microfrontend-composer).
...
You need also to specify the name of the `producer` as a string parameter: this information will be displayed in the [Data Catalog UI Job Details section.](/products/data_catalog/frontend/data_lineage.mdx#job-details)
...
Finally, you can add the endpoint `/ui/job-runner` (with rewrite base path as `/ui/job-runner`) having the permissions defined [in the Secure Access chapter](/products/data_catalog/secure_access.mdx#endpoints-2).
...
When instantiating [Data Catalog application](/runtime-components/applications/data-catalog/10_overview.md), Job Runner service configuration is generated with
...
:::tip
The following properties support [secrets resolution](/products/fast_data/configuration/secrets_resolution.md):
...
- the keys represent the __secret name__ that can be used by external services as reference when triggering a job. This name will be made available by the [gRPC method `ListSecretNames`](#configuration-1) to the [Data Catalog Frontend](/products/data_catalog/frontend/data_catalog_connections.mdx#input-management) during Connection creation;
- the values represent a secret (that can be represented according to [Fast Data secret resolution](/products/fast_data/configuration/secrets_resolution.md)).
...
and the [Connections UI](/products/data_catalog/frontend/data_catalog_connections.mdx) maps the `key` to the connection's `name` while the rest of the configuration is identical in case of a connection string.
...
[data-catalog-agent]: /runtime-components/templates/data-catalog-agent/10_overview.md
