...
- _template_ (only if input mode is _external_): a [configuration template](/products/microfrontend-composer/composer/20_compose_pages.md#templates) carrying a pre-defined, pre-arranged set of components.
...
- _edit page_ (only for compose pages): to open the section for [compose pages configuration](/products/microfrontend-composer/composer/20_compose_pages.md);
...
Some properties will be marked as **Shared**. This means that it is possible to assign them a common reusable property of the same type. In order to do so, create a Shared Property in the [Shared Properties](/products/microfrontend-composer/composer/20_compose_pages.md#shared-properties) section.
