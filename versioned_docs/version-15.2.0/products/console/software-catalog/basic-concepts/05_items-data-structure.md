...
On top of that, items have **[versions](/products/console/software-catalog/basic-concepts/20_items-versioning.md)** (filed `version.name`) and multiple instances of the same `tenantId` and `itemId` can occur for different `version.name` values.

:::info
To summarize, an item is **univocally defined** by the combination of its **`tenantId`**, **`itemId`**, and **`version.name`** fields.
:::

Another defining property of an item is the **type** (field `type` and `itemTypeDefinitionRef`) that determines how the item is handled by the Console. Types are defined and described by **[Item Type Definitions](/products/console/software-catalog/basic-concepts/10_items-types.md)**, another kind of user-controlled core Catalog entity.
...
An item lives through different **[development phases](/products/console/software-catalog/basic-concepts/30_items-lifecycle.md)** (field `lifecycleStatus`), and its **[availability](/products/console/software-catalog/basic-concepts/40_items-visibility.md)** (field `visibility`) can be controlled for a better distribution.
