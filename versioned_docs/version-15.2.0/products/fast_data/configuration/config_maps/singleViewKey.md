...
- `sv_id` is the name of the Single View field (ideally the primary key defined in the [Single View Data model page](/products/fast_data/configuration/single_views.md#single-view-data-model)) 
- `ID_USER` is the field name expected inside the Projection Changes document that triggers the update of the Single View. Most of the time this field will be the same name as the field inside the projection from which will start the aggregation of the Single View, but it might differ in case a [Projection Changes Schema](/products/fast_data/configuration/config_maps/projection_changes_schema.md) with customized names has been created.
...
:::info
Before to configure the Single View Key, the [ER Schema](/products/fast_data/configuration/config_maps/erSchema.md) and the [Aggregation](/products/fast_data/configuration/config_maps/aggregation.md) must be configured as well.
:::
