...
Using an enabled provider, a client executes an initial payments where they also authorize the underlying
system to perform future payment on their behalf, using the same details of the initial payment.
...
which can be either `true` or `false` respectively, different logics are be applied to the request.
