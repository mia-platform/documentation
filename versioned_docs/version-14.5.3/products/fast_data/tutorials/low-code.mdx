...
* basic knowledge about [Fast Data architecture](/products/fast_data/concepts/architecture.md), in particular:
  * [Standard Architecture](/products/fast_data/concepts/architecture.md#standard-architecture)
  * [Event-Driven Architecture](/products/fast_data/concepts/architecture.md#event-driven-architecture)
* familiarity with the concepts of:
  * [Projections](/products/fast_data/configuration/projections.md)
  * [Single View](/products/fast_data/configuration/single_views.md)
  * [Strategies](/products/fast_data/configuration/strategies.md)
...
If you're using a Console version lower than `v11.7.0`, you can skip the [System of Record Microservices section](#system-of-records-microservices) and just configure the [required environments variables](/products/fast_data/configuration/realtime-updater/realtime-updater.md) of the micro-service.
:::
...
You can choose between two microservices: the [Real-Time Updater](/products/fast_data/realtime_updater.md) or the [Projection Storer](/products/fast_data/projection_storer.md).
...
* [Kafka Consumer](/products/fast_data/configuration/projection_storer.md#consumer), which will be used to read [ingestion-messages](/products/fast_data/concepts/inputs_and_outputs.md#ingestion-message),
* [Kafka Producer](/products/fast_data/configuration/projection_storer.md#producer), which will be used to generate [pr-update messages](/products/fast_data/concepts/inputs_and_outputs.md#projection-update-message),
* [Storage Configuration](/products/fast_data/configuration/projection_storer.md#storage), which will contain the MongoDB connection where the database is located.





Real-Time Updater Setup
...
:::info
The Real-Time Updater includes more optional parameters that might be included via Environment Variables. If you need to know more about them, you can [read the documentation of the Real-Time Updater](/products/fast_data/configuration/realtime-updater/realtime-updater.md).
:::
...
* [Ingestion topic](/products/fast_data/concepts/inputs_and_outputs.md#ingestion-message):  will be used by the SoR microservice
to receive the data from the Source Systems so that the Projections will be updated on the target Database;
* [PR Update topic](/products/fast_data/concepts/inputs_and_outputs.md#projection-update-message): will be used to notify when a projection has been updated.
...
Now it's time to [save](/products/console/handbooks/project-usage.md#save-changes) and [deploy](/products/console/handbooks/project-usage.md#deploy-changes) the new configuration.
![Deploy the configuration](img/10-deploy.png)

After that, we can try to send a message to the _Kafka Ingestion Topics_ created in the previous steps, and we can see the data stored in the Projection using the APIs available in the [API Documentations](/products/console/project-configuration/documentation-portal.md).

Since we have chosen `Debezium` as the format for our _SoR_, the ingestion message that we will send has to comply to [its specifications](/products/fast_data/concepts/inputs_and_outputs.md#debezium).
...
:::tip
If you don't have a direct access to your Kafka broker to produce messages, you can use a dedicated microservice such as [Rest2Kafka](/runtime-components/plugins/rest2kafka-service/01_overview.md) to produce Kafka messages as REST request directly from MIA Console.
:::
...
We will go for the latter, since we want to leverage the usage of [Low Code / No Code features](/products/fast_data/no_code_overview.md#what-are-low-code-and-no-code) for a quicker configuration of the microservice. We suggest naming it `single-view-creator-delivery`.
...
- `SINGLE_VIEWS_ERRORS_COLLECTION`: a collection where the service will write errors that may happen during aggregation. (we suggest `order-history-errors`, [here](/products/fast_data/concepts/inputs_and_outputs.md#single-view-error) you can read about record format that will be stored)
...
* kafka connection variables such as:
  ...
  * `KAFKA_PROJECTION_CHANGES_TOPICS`: name of the topic that will receive projection changes in the shape of [trigger messages](/products/fast_data/concepts/inputs_and_outputs.md#single-view-trigger-message), you can choose `demo.development.delivery.order-history.trigger`;
...
:::info
The Single View Creator includes more optional parameters that might be included via Environment Variables. If you need to know more about them, you can [refer to the related documentation section](/products/fast_data/configuration/single_view_creator/index.md#environment-variables).
:::
...
The configuration of the ER Schema with the "No Code" is explained in the [dedicated section](/products/fast_data/configuration/config_maps/erSchema.md#use-the-no-code),
...
To configure the _Single View Key_ with the "No Code" feature, you can refer to the [specific documentation page](/products/fast_data/configuration/config_maps/singleViewKey.md#using-the-no-code).
...
:::tip
To have an exhaustive explanation of how to configure the _Single View Key_ configuration with the **Low Code** feature, you can refer to the [specific documentation page](/products/fast_data/configuration/config_maps/singleViewKey.md).
:::
...
:::tip
To have an exhaustive explanation of how to configure an Aggregation with the **No Code** feature, you can refer to the [specific documentation page](/products/fast_data/configuration/config_maps/aggregation.md#use-the-no-code).
:::
...
:::tip
To have an exhaustive explanation of how to configure an Aggregation with the **Low Code** feature, you can refer to the [specific documentation page](/products/fast_data/configuration/config_maps/aggregation.md).
:::
...
* if you have defined a _RTU_ having the following environment variables:
  ...
  then you are using a [Standard Architecture](/products/fast_data/concepts/architecture.md#standard-architecture) where strategies are executed within the _Real Time Updater_;
...
* if you have defined a _RTU_ having the following environment variables:
  ...
  or you have configured a _PS_, then you are using a [Event Driven Architecture](/products/fast_data/concepts/architecture.md#event-driven-architecture), where strategies are executed by an additional microservice, the **Single View Trigger Generator** (**SVTG**).
...
:::tip
You can find a detailed explanation of the _Projection Changes Schema_ configuration on the [dedicated page](/products/fast_data/configuration/config_maps/projection_changes_schema.md) of the documentation.
:::
...
With the Event Driven Architecture, strategies are executed after a `pr-update` message by a dedicated microservice, the [Single View Trigger Generator](/products/fast_data/single_view_trigger_generator.md). (**SVTG**)
...
:::tip
You can find a detailed explanation of the _Kafka Projection Updates_ configuration on the [dedicated page](/products/fast_data/configuration/config_maps/kafka_projection_updates.mdx) of the documentation.
:::
...
Now we can [commit](/products/console/handbooks/project-usage.md#save-changes) our changes, [deploy](/products/console/handbooks/project-usage.md#deploy-changes), and try our [Fast Data](/products/fast_data/concepts/architecture.md) sending data from the Kafka Topic, and see the result in the Single View using the endpoints.

### Test your Single View

You can now consume the order-history Single View using the APIs available in the [API Documentations](/products/console/project-configuration/documentation-portal.md).
