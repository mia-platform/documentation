...
Fast Data Runtime Management solution has a dedicated frontend that allows interacting with [Control Plane](/products/fast_data/runtime_management/control_plane.mdx) via [Fabric BFF](/products/fast_data/runtime_management/control_plane_fabric_bff.mdx)
component.
...
Users with enough permissions can manage [Runtime Views](/products/fast_data/runtime_management/overview.mdx#runtime-view) inside Control Plane frontend.  
...
In particular, two types of pipelines are shown: [Domain pipelines](/products/fast_data/runtime_management/overview.mdx#domain-pipeline) and [Single View pipelines](/products/fast_data/runtime_management/overview.mdx#single-view-pipeline).
...
:::tip 
Bulk actions are useful in case of large data pipelines and to speed up runtime management: for example, they can be leveraged during an [Initial Load management process](/products/fast_data/concepts/data_loading.mdx#initial-load).
...
Bear in mind that changing the `BASE_PATH` environment variable requires to change also the [rewrite base path](/products/console/api-console/api-design/endpoints.md#about-rewrite-base-path) of exposed endpoint to `/<your-base-path>`.

### Endpoints

In order to allow incoming traffic from outside to Control Plane Frontend, it is necessary to configure the main service endpoint. This can be configured in the `Endpoints` page of Console Design area, as explained [here](/products/console/api-console/api-design/endpoints.md).
...
Consequently, it is necessary to carry out a manual update of your API Gateway configuration in Console [Advanced Section](/products/console/api-console/advanced-section/index.md).
...
For more details on the websocket endpoint, please read the [endpoint paragraph](/products/fast_data/runtime_management/control_plane.mdx#endpoints) of Control Plane service documentation.
...
-  is specific for the `frontend` listener of Envoy. In case it has been decided to expose the Fast Data Control Plane
  system under a [different listener](/products/console/api-console/api-design/listeners.md), remember to update the configuration accordingly.
:::
...
Fast Data Control Plane UI can be embedded as an extension of Mia-Platform Console using [Mia-Platform Platforge Extensions](/products/console/console-extensibility/overview.md).
...
In order to achieve so, Mia-Platform Console offers an integrated tool for [managing extensions](/products/console/company-configuration/extensions.md) that streamlines the registration procedure.
