...
Visit **[this page](/products/fast_data/concepts/architecture.md)** to learn more about the architecture of Mia-Platform Fast Data.
...
Mia-Platform Fast Data utilizes an event streaming platform to ingest messages about changes within your systems (e.g., create, update, or delete operations) in near real-time. The components responsible for transmitting these changes to the event streaming platform are known as **[connectors](/products/fast_data/connectors/overview.md)**.
...
You can either develop a custom connector tailored to your specific needs or use one of the connectors available in the **[Mia-Platform Marketplace](/runtime-components/overview_marketplace.md)**.
...
For further information about our No Code / Low Code logic, please visit the **[dedicated page](/products/fast_data/no_code_overview.md)**.
...
For more information about Control Plane configuration, visit **[this page](/products/fast_data/runtime_management/control_plane.mdx)**. 
...
* **[Data Catalog](/products/data_catalog/overview.mdx)**: a command-line interface binary designed to query data sources for their schemas and transmit them to a target location. Data Catalog acts as a driver for collecting and unifying views of tables and collections from various sources (SoRs).
* **[Bucket Storage Support](/products/fast_data/bucket_storage_support/overview.md)** allows messages passing through a channel to be stored in an external storage system, such as a Google Cloud Storage bucket or any bucket compatible with the Amazon AWS S3 interface.
* **[Flow Manager Service](/products/console/tutorials/configure-marketplace-components/flow-manager.md)**: a saga orchestrator, capable of managing workflows structured using the Saga Pattern architectural framework.
* **[CRUD Service](/runtime-components/plugins/crud-service/10_overview_and_usage.md)**: its purpose is to abstract Data Collections, enabling developers to easily, scalably, and securely expose CRUD APIs over the database.

Visit the **[Mia-Platform Marketplace](/runtime-components/overview_marketplace.md)** page where you can find other useful ready-to-use plugins.
