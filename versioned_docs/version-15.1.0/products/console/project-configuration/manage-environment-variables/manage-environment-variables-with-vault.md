...
1. Create a provider for your Vault instance. This can be done using the dedicated [providers APIs](/products/console/company-configuration/providers/configure-provider.mdx). The provider is the entity that contains references to the public hostname of your Vault instance and credentials to access its APIs. 
  
  In this step you have to configure:
    - The [provider related fields](/products/console/company-configuration/providers/configure-provider.mdx?providerType=vault#step-2-provider-details), having type `vault`. Optionally, you can setup a custom Certificate Authority and/or proxy.
