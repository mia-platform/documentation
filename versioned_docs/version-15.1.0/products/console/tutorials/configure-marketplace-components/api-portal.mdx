...
In this tutorial, we will learn how to configure and use the [_API Portal_](/runtime-components/plugins/api-portal/10_overview.md) and the [_Swagger Aggregator_](/runtime-components/plugins/swagger-aggregator/10_overview.md) in order to provide the developers with a user-friendly interface that enables them to easily visualize and test endpoints of different microservices in a single place. It is built on top of the [OpenAPI 2.0](https://swagger.io/specification/v2/)/[OpenAPI 3.0](https://swagger.io/specification/v3/)/[OpenAPI 3.1](https://swagger.io/specification/) Specification.

## What we will build

We will add the _API Portal_ and _Swagger Aggregator_ [_Marketplace_ plugins](/runtime-components/plugins/mia-platform-plugins.md) in a project using the dedicated _Application_. As a result, we will use the Portal to view and test project endpoints.
...
:::info
If you have already done the [previous tutorial](/products/console/tutorials/configure-marketplace-components/microservice-hello-world.mdx), you can skip this section and go on using the same project.
:::

Before starting, we will assume that, you already have a clean project in Mia-Platform Console. In order to know how to create a project on Mia-Platform Console, read [this guide](/products/console/project-configuration/create-a-project.mdx).

The project must:

1. Be integrated with a [deploy pipeline](/requirements/installation-guidelines/console/self-hosted/pipeline-configuration.md);
1. Have an [ingress route](/requirements/installation-guidelines/console/paas/tools/traefik.md) with `api-gateway` as `service`;
1. Have an [API Gateway](/runtime-components/plugins/api-gateway/10_overview.md) or a [Envoy API Gateway](/runtime-components/plugins/envoy-api-gateway/overview.md) in your project;
...
In order to start the tutorial, you must be in the _design_ section of the project in the [Microservices section](/products/console/api-console/api-design/services.md) from the left side menu.
...
To do so we need to include in our project an [_API Portal_](/runtime-components/plugins/api-portal/10_overview.md) and a [_Swagger Aggregator_](/runtime-components/plugins/swagger-aggregator/10_overview.md) service.
...
- [_API Portal_](/runtime-components/plugins/api-portal/10_overview.md), that will present a graphical interface to show the OpenAPI documentation of our endpoints;
- [_Swagger Aggregator_](/runtime-components/plugins/swagger-aggregator/10_overview.md), that will be contacted by the _API Portal_ to have the entire OpenAPI documentation to be shown. In order to create it, the _Swagger Aggregator_ will retreive the OpenAPI documentation of each service and merge it into a single one.
- you can choose to create a new _API Gateway_ service or choose an already installed service.

and the following endpoints:
- _/documentations/api-portal_ to the [_API Portal_](/runtime-components/plugins/api-portal/10_overview.md) service
- _/api/openapi_ to the [_Swagger Aggregator_](/runtime-components/plugins/swagger-aggregator/10_overview.md) service which reroutes to `/openapi/v3-1`
- _/api/openapi/raw_ to the [_Swagger Aggregator_](/runtime-components/plugins/swagger-aggregator/10_overview.md) service
...
:::tip
If you do not know how to do it, take a quick look at [Mia-Platform handbook](/products/console/handbooks/project-usage.md#deploy-changes).
:::

## Open the interface and test the endpoint

The last step is to open the API portal interface. Firstly, go to the [_environments_](/products/console/project-configuration/manage-environment-variables/index.md) section of the _Project Overview_ area. Now open the Environment's menu item and select "Go to Documentation" of the wanted environment, as shown in the image below:
