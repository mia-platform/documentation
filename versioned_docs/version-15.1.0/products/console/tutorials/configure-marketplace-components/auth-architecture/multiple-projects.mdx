...
For example, you could replicate [Scenario 2](/products/console/tutorials/configure-marketplace-components/auth-architecture/external-idp.md) as in the following schema:
...
1. Create the `Edge Gateway` project following the [project creation documentation](/products/console/project-configuration/create-a-project.mdx)
2. Configure one [cross-project proxy](/products/console/api-console/api-design/proxy.md#type-cross-projects) for each project
...
4. Configure the `API Gateway` to forward the HTTP headers resulting from the authentication flow (`Miauserid`, `Miausergroups`, `Miauserproperties`) to the projects' `API Gateway`
    - Follow [this guide](/products/console/api-console/advanced-section/api-gateway/how-to.md#how-to-manage-authorization-in-a-multi-gateway-architecture) if you have Nginx API Gateway
    - Follow [this guide](/products/console/api-console/advanced-section/api-gateway-envoy/jwt-authn.md) if you have Envoy API Gateway
5. Create an authentication manager as described in [Scenario 2](/products/console/tutorials/configure-marketplace-components/auth-architecture/external-idp.md)
...
1. Create the [`Authorization Service`](/runtime-components/plugins/authorization-service/10_overview.md) plugin from Marketplace
    - This step can be done as described in [Scenario 2](/products/console/tutorials/configure-marketplace-components/auth-architecture/external-idp.md)
    - The only important difference is that the `Authorization Service` must be configured in ["Trust Mia Headers" mode](/runtime-components/plugins/authorization-service/30_usage.md#trust-mia-platform-user-headers) by setting the `TRUST_MIA_USER_HEADERS` environment variable to `true`
2. Secure your endpoint as described in [Scenario 2](/products/console/tutorials/configure-marketplace-components/auth-architecture/external-idp.md)

### Centralize API Keys in the Edge Gateway

You can optionally choose to centralize in the Edge Gateway all the API keys.
1. Define your API Keys in the `Edge Gateway` as described in [Scenario 1](/products/console/tutorials/configure-marketplace-components/auth-architecture/static-api-key.md)
