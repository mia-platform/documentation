...
## Quick start
After the [installation](/runtime-components/libraries/kafka-healthchecker/30_setup.md), you can import the library, create the `KafkaJSHealthChecker` object passing it one (or more) consumer or producer to get the methods `isHealthy` and `isReady`. More information about these methods can be found on [Overview](/runtime-components/libraries/kafka-healthchecker/10_overview.md) page.
