...
The Kafka Projection Updates is a JSON file which describes for each projection a [Projection Updates](/products/fast_data/concepts/inputs_and_outputs.md#projection-update) topic and a [Strategy](/products/fast_data/configuration/strategies.md) to generate the projection changes identifier. This file is typically used by:

* the [Real-Time Updater](/products/fast_data/realtime_updater.md), to generate a `pr-update` message;
* the [Single View Trigger Generator](/products/fast_data/single_view_trigger_generator.md), to execute a strategy starting from a `pr-update` message.
...
- `updatesTopic`: The topic where the Projection Updates messages are sent, if you haven't created a topic for the projection yet take a look to our [naming conventions for the Projection Updates topics](/products/fast_data/concepts/inputs_and_outputs.md#topic-naming-convention-1).
...
The automatic strategy handles the generation of the projection changes identifier accordingly to [the paths in the Projection Changes Schema](/products/fast_data/configuration/config_maps/projection_changes_schema.md) config map containing the projection.
...
:::info
The `file` should be mountend into a specific config map of the micro-service. Please refer to [Real-Time Updater](/products/fast_data/configuration/realtime-updater/realtime-updater.md) and [Single View Trigger Generator](/products/fast_data/configuration/single_view_trigger_generator.mdx) configuration pages.
:::
...
- `updateEvent`: The Update Event object created from the [Projection Update](/products/fast_data/concepts/inputs_and_outputs.md#projection-update-message) message.
