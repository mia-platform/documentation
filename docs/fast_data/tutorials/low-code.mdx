...
Altought the two services have several differences, that we will see later during strategy handling, they have one shared feature: they read ingestion messages from Kafka topics to store them into the projections on MongoDB. 

<details>
<summary><strong>Projection Storer Setup</strong></summary>
<p>
...
<details>
<summary><strong>Real-Time Updater Setup</strong></summary>
<p>
...
Going into the _fast-data-delivery-realtime-updater_ microservice page, we have to configure the following resources:
...
![Link ER Schema](./img/38-link-erschema.png)
...
Now, we will choose the System Of Records `delivery` and the `registry` Projection.
...
Also, you have to configure the required property `EVENT_STORE_TARGET`, which represents the 
</TabItem>
</Tabs>
