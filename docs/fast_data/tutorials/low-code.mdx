...
The architectural component responsible for creating and updating the projection is either the _Real-Time Updater_ or, starting from `Console v12.0.0`, the _Projection Storer_: we will refer to them as `SoR Microservices` throughout this tutorial.

This microservice manages a Kafka consumer that consume the data from the source systems and update the corresponding projections on MongoDB.
