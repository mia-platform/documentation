...
Below is reported the set of main error codes that may occur during the configuration and service runtime operations. Each of them
is associated with a brief explanation on how to potentially resolve the encountered issue.
...
| FD_SVTG_E101 | Runtime       | no strategy associated to topic                                   | in the config map loaded from `KAFKA_PROJECTION_UPDATES_FOLDER` verify that each topic the service is subscribed to is mapped to one or more strategies                                                                                                                                                                                                       |
| FD_SVTG_E102 | Runtime       | a message with null payload has been received                     | verify the topic channel on which such event happened to understand whether this event was generated from the events producers or it was received by mistake                                                                                                                                                                                                  |
