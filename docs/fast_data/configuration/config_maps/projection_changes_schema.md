...
The Projection Changes Schema is a JSON file (`projectionChangesSchema.json`) which helps the [strategy](/fast_data/concepts/the_basics.md#strategies) to find the right path from the [initial projection](/fast_data/concepts/glossary.mdxx)
to the [base projection](/fast_data/concepts/glossary.mdxx) and then to the identifier used to match the single view document. This file is typically used
