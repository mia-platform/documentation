...
2. **Set Up Fast Data Environment in Console:** naturally, you need to have a Fast Data environment in the Console,
  ...
  In particular, be sure to set the three environment variables `CONTROL_PLANE_ACTIONS_TOPIC`, `CONTROL_PLANE_KAFKA_GROUP_ID`, and `READ_TOPIC_FROM_BEGINNING` for the Single View Creator,
  to enable the communication with the Runtime Management.
  Check the Single View Creator [Environment Variables section](/fast_data/configuration/single_view_creator.md#environment_variables) for further details.

  :::tip
  For an overview of the Fast Data architecture and what services can be deployed, check the [Architecture](/fast_data/architecture.md) page or consult the [Fast Data Tutorial](/getting-started/tutorials/fast-data/low-code.mdx) for a step-by-step guide on how to create a Fast Data environment.
  :::
