...
To configure the Control Plane connection for the Projection Storer, first [you need to attach the microservice to a System of Record](/fast_data/configuration/projection_storer_config#attach-to-system-of-record).

Then, you can scroll over the Control Plane section and parse your JSON configuration.
...
Then, workloads should have the `settings` properties like the following:
...
The following fields contains secrets that can be managed using the [Fast Data Secret Resolution mechanism](/fast_data/secret_resolution.md):
...
Once a Control Plane configuration has been defined for one of the workloads defined above, upon saving,
...
- for other microservices, an additional environment variable `CONTROL_PLANE_BINDINGS_PATH` managed by
  Mia Console will be set-up with the full path of the new configuration.   
