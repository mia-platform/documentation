...
This application can be found in the Marketplace by looking it up in using the keyword `control plane`, which will present you with two options for instantiating its configuration:

- _Fast Data Control Plane_, which will provide the basic Control Plane instance alongside an Envoy API Gateway.
- _Fast Data Control Plane With Access Control_, which will provide all the configurations of _Fast Data Control Plane_ with the addition of the resources needed for introducing an authorization layer (see box tip below for more information)
...
Below is shown the Marketplace application page where the term `control plane` has been looked up. In case this application is not available in your Marketplace, please contact your Console administrator to verify the Fast Data license is active for your company.

![Control Plane Marketplace](img/marketplace_runtime_management_app_selection.png)

Among the services provided by both applications, there are the two services in charge of deploying the Fast Data Runtime Management solution, which are:
...
Once the Control Plane is configured, it is then necessary to configure and deploy in each Fast Data Runtime namespace a new service, the Control Plane Operator (ADD LINK TO CONFIG).
...
The Control Plane Operator is available from the Mia-Platform Marketplace as a plugin, which comes with a blueprint of the configuration to speed up its setup.
...
This concepts are in addition the ones already provided by [Fast Data Section](/fast_data/concepts/glossary.mdx). 
...
Within the context of Fast Data Control Plane, a _runtime_ corresponds to [a deploy environment of Mia Console](/development_suite/monitoring/introduction.md),
where an instance of Control Plane Operator has been deployed. In particular, a runtime can be identified across Fast Data Control Plane solution by the triple:
...
compose a _runtime_ for the Control Plane.
...
Within the context of Fast Data Control Plane, a _runtime view_ corresponds to a group of one or more [_runtimes_](#runtime) that are logically clustered together. These grouping
may either arise from business requirements or spontaneously depending on how Console projects are linked among them.
