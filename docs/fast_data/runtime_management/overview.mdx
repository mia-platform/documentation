...
Control Plane application (**LINK TO THE MARKETPLACE ITEM DOCS**) can be found in the Marketplace by looking it up in using the keyword `control plane`, which will present you with an option for instantiating its configuration.
...
The Control Plane Operator is available as a [Mia-Platform Marketplace plugin](/plugins/mia-platform-plugins.md), which comes with a blueprint of the configuration to speed up its setup.
