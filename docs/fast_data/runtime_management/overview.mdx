...
Here are listed some concepts that are used on the Runtime Management solution. This concepts complement the ones already provided by [Fast Data Section](link). 
...
- **Reducing** <img src="/img/fd-cp-reducing-step-icon.svg"/>: is the step involved to compute the identifier of the curated data that is aggregated by a pipeline. 
  ...
  For runtimes implementing the [Standard Architecture](link), i.e. having a Real-Time Updater that performs the execution of [strategies](link),
  the Reducing step cannot be controlled since no SVTG is employed.  
  :::
