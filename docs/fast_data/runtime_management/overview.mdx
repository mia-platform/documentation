...
Here are listed some concepts that are used on the Runtime Management solution. This concepts complement the ones already provided by [Fast Data Section](/fast_data/glossary.mdx). 

### Workload 

A _workload_ represent a microservice of a Fast Data architecture that can be controlled through Control Plane.  
...
At the moment, a Control Plane instance can manage only the pipeline microservices deployed on that specific runtime.
...
An _artifact_ represents a unique entity inside the pipeline. This entity can be of different types, dependending on its occurrence inside the pipeline, which are:
...
- **CDC**: is the step that register changes from the data source of a pipeline, sending the corresponding operations to an ingestion topic. 
  This step can't be controlled by the Control Plane.
