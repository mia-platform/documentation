...
* send to the main Control Plane instance the workloads state (e.g. whether they are up-and-running, _active_ or _paused_) with respect to the [Fast Data Pipeline](/fast_data/runtime_management/overview.mdx#pipeline) they are linked to;
...
| `MIA_PLATFORM_EU_TENANT`             | &check;  | Identifier of the company that contains the project where this service is configured.<br/>This is retrieve exploiting [Downward API](/development_suite/api-console/api-design/services.md#from-downward-api) configuration |                                          |
| `APP_KUBERNETES_IO_PART_OF`          | &check;  | Identifier of the project where this service is configured.<br/>This is retrieve exploiting [Downward API](/development_suite/api-console/api-design/services.md#from-downward-api) configuration                           |                                          |
| `MIA_PLATFORM_EU_STAGE`              | &check;  | Environment where this service is deployed to.<br/>This is retrieve exploiting [Downward API](/development_suite/api-console/api-design/services.md#from-downward-api) configuration                                        |                                          |

:::tip
This is how environment variables using Downward API should look like under Control Plane Operator microservice page of Console Design section:
...
The `url` can use both `http` and `https` protocol, although the formed is deduced as default. The latter can be configured as default connection
...
when it recognizes it as a known plugin, that is Control Plane Operator service has been instantiated from Mia-Platform Marketplace.
...
Thus, it is necessary to advertise the port where the gRPC controller is exposed by Control Plane service, which by default is the `50051`.
...
that can be found in the Console Design section, under the specific microservice resource. In the image below it is shown how
...
The information that can be extracted from these messages are:

- which artifacts are up-and-running;
- which workloads have already successfully reconciled their state with the one request in the latest _State Message_;  
