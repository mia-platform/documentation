...
* receive user interactions through the dedicated frontend, and to update pipeline state accordingly;
...
| `/fast-data/feedback` | Websocket | GET    | Opens a websocket connection with the client to receive updates of runtimes and pipelines              |
...
When using the ControlTo ensure that websocket communications work properly, it is necessary to configure the additional manual operations must be performed in the [Advanced Section](/development_suite/api-console/advanced-section/index.md) of your gateway.
...
The [Control Plane applications](/runtime_suite_applications/fast-data-control-plane/10_overview.md) provide a sample of this JSON in a dedicated Config Map, named `control-plane-configuration`.
