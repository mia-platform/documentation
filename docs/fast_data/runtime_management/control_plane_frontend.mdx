...
Runtime Management has a dedicated frontend that interacts with [Control Plane](/fast_data/runtime_management/control_plane.mdx) via the [Fabric BFF](/fast_data/runtime_management/control_plane_fabric_bff.mdx).

This frontend enable the user to:
...
|--------------|----------|--------------------------------------------------------------|-------------------|
| `BASE_PATH`  | &check;  | Path prefix to be added to all the frontend resources        | `/control-plane/` |
| `COMPANY_ID` | &check;  | The identifier of the company from which runtimes are loaded |                   |

As effect of them, by default the Control Plane application exposes its frontend over the `/control-plane/` base path.
...
:::info
The `COMPANY_ID` environment variable is quite relevant, since it indicates which projects can be potentially accessed and managed by Control Plane application.
To retrieve it, either contact your Company Owner or open your Company Overview page and observe the URL displayed in the browser search bar.
The `company-id` value is the one set in the path `<console-base-url>/tenants/:company-id/projects`.
:::

### Endpoints

In order to allow incoming traffic from outside to Control Plane Frontend, it is necessary to configure the main service endpoint. This can be configured in the `Endpoints` page of Console Design section, as explained [here](/development_suite/api-console/api-design/endpoints.md).
...
where the websocket communications should be enabled. In case `Fabric BFF` service exposes the feedback endpoint elsewhere, please update the configuration accordingly, if needed.

For more details on the selected endpoint, please read the [endpoint paragraph](/fast_data/runtime_management/control_plane.mdx#endpoints) of Control Plane service documentation.
...
-  is specific for the `frontend` listener of Envoy. In case it has been decided to expose the Fast Data Control Plane
  system under a [different listener](/development_suite/api-console/api-design/listeners.md), take into account of updating the configuration accordingly.
:::
...
Fast Data Control Plane UI can be embedded as an extension of Mia-Platform Console using [Mia-Platform Platforge](/release-notes/mia-platform-v13.mdx#platforge-enhance-extensibility).
...
When users access Control Plane frontend, an overview section shows the list of Fast Data pipelines released on that runtime environment.
...
one example is in case of an [Initial Load management process](/fast_data/concepts/data_loading.mdx#initial-load). 
