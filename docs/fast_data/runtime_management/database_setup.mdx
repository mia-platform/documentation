...
Running the CLI inside your project can be done exploiting a K8s cronjob. In this [documentation page](/development_suite/api-console/api-design/jobs-cronjob.md) it is described how it can be configured, while here
are highlighted which specific configurations of the K8s manifest:
...
- `<TAG>` &rarr; is the latest stable version of CLI image;
    ...
    spec:
      ...
      template:
        ...
        spec:
          ...
          containers:
            - name: fabric-control-plane-admin
              image: <URL_TO_CONTAINER_REGISTRY>/data-fabric/fabric-admin:<TAG>
              args: [
                "all",
                "init",
                "--url",
