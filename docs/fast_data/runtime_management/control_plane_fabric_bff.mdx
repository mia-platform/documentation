...
For example, it could be possible to assign the role of `guest` at Company level while granting
the `reporter` role to all the project that should be visible by Control Plane.
