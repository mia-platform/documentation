...
:::tip Service account permissions assignment
...
About permissions assignment, it is possible to go even more granular in case you want to allow visibility only to a subset of runtime environments of a specific project; in fact, to do that, you may opt to assign the role of `guest` even at Project level while granting
the `reporter` role solely to those runtime environments that should be visible by Control Plane for a specific project.
:::
