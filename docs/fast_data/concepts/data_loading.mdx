...
There are scenarios in which it is required to regenerate the kafka update messages starting from the projection (pr-update)
or the single-view (sv-update) collections.

This script will generate update messages based on their specification.
You can generate both projection update and single view update messages.
...
You will have to change the following variables:
- collection: projection or single view collection
- updateTopic: pr-update or sv-update topic
- typeUpdate: "pr-update" or "sv-update"
- messageVersion: "v1.0.0" or "v2.0.0"
- mongoUrl: mongodb string connection
- brokers: array of string with the list of brokers
- sasl: sasl configuration; username, password and mechanism.
- indexName: the primary index name
...
function getUpdateMessage({doc, primaryKeys, projectionName, operationTimestamp, typeUpdate, messageVersion}) {
    if (messageVersion === 'v2.0.0') {
        return {
            key: pick(primaryKeys, doc),
            value: {
                operation: {
                    "type": "INSERT",
                    "timestamp": operationTimestamp
                },
                "storageNamespace": projectionName,
                source: "script",
                primaryKeys,
                before: null,
                after: doc,
            ...
            headers: {
                messageSchema: {"type":typeUpdate,"version":"v2.0.0"}
            }
        }
    }
    if (messageVersion === 'v1.0.0') {
        return {
            key: pick(primaryKeys, doc),
            value: {
                ...
                primaryKeys,
                operationTimestamp,
                __internal__kafkaInfo: doc.__internal__kafkaInfo,
                projectionName,
                source: "script",
                before: null,
                after: doc,
    ...
    throw new Error('message version not valid')
}

async function main(projectionName, topic, typeUpdate, messageVersion, parallelRequests = 50) {
    const mongoUrl = ""
    ...
    const kafka = new Kafka({
        brokers: [/** ... put here your kafka brokers ... */],
        clientId: `mia.local.update-generator`,
        // SSL Configuration
        ssl: {
            ca: await fs.readFile('ca.pem', 'utf-8'), // required if ca.pem file is needed for kafka
        },
        /** replace with your authentication mechanism */
        sasl: {
            username: '',
    ...
    try {
        ...
        const indexName = ''
        const primaryKeyIndex = indexes.find(index => index.name === indexName)
        const primaryKeys = Object.keys(primaryKeyIndex?.key)

        if (!primaryKeyIndex) {
            throw new Error('no pk index found')
        }
        ...
        for await(doc of cursor) {
            const message = getUpdateMessage({doc, primaryKeys, projectionName, operationTimestamp, typeUpdate, messageVersion})
            messages.push({
                key: JSON.stringify(message.key),
                value: JSON.stringify(message.value),
                headers: JSON.stringify(message.headers)
            ...
            if (messages.length === parallelRequests) {
                await Promise.resolve(producer.send({
                    topic,
                    messages,
                    // if many messages are involved, consider using GZIP compression
...
const collection = '' // projection or single view collection
const updateTopic= '' // pr-update or sv-update topic
const typeUpdate = '' // 'pr-update' or 'sv-update'
const messageVersion = '' // "v1.0.0" or "v2.0.0"

main(collection, updateTopic, typeUpdate, messageVersion)
```

In the kafka configuration please take in consideration the ssl field. In the script is required the `ca.pem` file.
