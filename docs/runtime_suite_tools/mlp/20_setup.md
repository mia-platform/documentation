...
```sh
go install github.com/mia-platform/mlp@v1.2.3
```

Or like this if the `install` command is not available

```sh
go get -u github.com/mia-platform/mlp@v1.2.3
```
...
```sh
curl -fsSL https://github.com/mia-platform/mlp/releases/download/v1.2.3/mlp-linux-amd64 -o /tmp/mlp
```

```sh
wget -q https://github.com/mia-platform/mlp/releases/download/v1.2.3/mlp-linux-amd64 -O /tmp/mlp
```
...
```sh
docker run ghcr.io/mia-platform/mlp:v1.2.3
```
