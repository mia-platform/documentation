...
A new `--public` flag has been added to the `miactl marketplace list` command: it allows to fetch and visualize not only the items of the current requested Company, but also all the public items from other Companies.

Check the [`miactl marketplace list` command documentation](/cli/miactl/commands.md#list-5) for more information.
