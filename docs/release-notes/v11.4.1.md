...
* During merge process, inside each file users can now accept all incoming or current changes instead of solving diff by diff. Thanks to two proper buttons, it is now possible to decide to either keep current changes (on the right-hand editor) or accept incoming changes (on the left-hand editor).
