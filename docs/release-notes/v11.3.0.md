...
From now on, within the CMS you can configure `defaultAnnotations` and `defaultLabels` properties for your Marketplace microservices. For more information about microservice properties, visit the [related page](../marketplace/add_to_marketplace/contributing_overview#common-to-microservice-components-plugins-templates-examples).
...
Do not miss out the [official documentation](/fast-data/configuration/config_maps/aggregation#use-the-no-code).
...
For on-premise installations, please contact your Mia-Platform referent and upgrade your _Console Helm Chart_ to `v9.3.0`.
