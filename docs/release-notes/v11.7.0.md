...
From now on, every user will be able to leave feedback directly in the console to suggest an idea, comment, report some small problem or anything else that might be useful.

The feedback will be addressed to the Console Managers who will be able to conveniently manage and moderate them through a new section of the CMS, in order to constantly improve the product experience.
...
On the miactl CLI, new runtime commands will be available from this release that will allow many of the runtime actions executable from the front-end of the Console to be simplified and brought via command line:
...
* Creation of a Job from a Cronjob.
