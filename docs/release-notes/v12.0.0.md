...
Similarly to Real-Time Updater plugin, Projection Storer consumes change events as ingestion messages,
transform them into projection records, filtering and validating each of their fields and eventually save them onto the specified storage system, emitting a proper notification for downstream components.
...
* Editors in advanced sections correctly hints and validates content against schemas
...
For each state of the machine it is now possible to specify an array of side effects command to file alongside the main command in a fire-and-forget manner.
...
Commands payload can now be generated thought a custom function that takes as input the entire Saga entity and the message label of the command, and returns as output an object with the payload to use.
