...
:::danger
If you have enabled the emission of pr-updates in a Real-Time Updater/Projection Storer but not all projections should emit the events, make sure you clear the newly created pr-update topic inputs inside the details page of each excluded projection.

![Kafka Topics section inside Projection details page](../fast_data/configuration/img/kafka_topics_projection_page.png)
