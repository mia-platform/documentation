...
This tool requires to link a Flow Manager service in order to actually edit its configuration, nevertheless unlinked services can still be managed editing the JSON configuration file as before. The link and unlink process are handled by the tool, but the choice is left to the user.
...
* fixed a bug on the webhook for user synchronization that was causing it to malfunction for user delete;
