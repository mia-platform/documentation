...
Start customizing your Console by adding your first iFrame extension, simply by selecting the Area and Sidebar section from those available and adding the entry URL you wish to add and make visible to your users! 
...
The `PUT /api/extensibility/tenants/{tenantId}/extensions` used to register extensions supports a new body format that is clearer and simple to use, along with the manifest to be sent using the miactl `extensions apply` command.  
