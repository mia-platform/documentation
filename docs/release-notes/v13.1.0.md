...
The body of the `PUT /api/extensibility/tenants/{tenantId}/extensions` used to register extensions has been modified and simplified, along with the manifest to be sent using the miactl `extensions apply` command.

:::danger
**The legacy schema is still supported until the next version**, so it is advisable to migrate to the new schema as soon as possible by following the instructions below.
:::

<details>
  <summary>Migration guide from legacy to new apply body schema</summary>

Starting from the following legacy schema as example:
...
it is possible to migrate to new schema properties following the mapping explained by this table:
...
After migration the new body schema is the following:
