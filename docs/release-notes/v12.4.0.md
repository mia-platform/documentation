...
### Decide whether to ask for confirmation before saving or to perform a quick save

With the introduction of tab `Rules` previously mentioned, we have implemented a first rule dedicated to improve users' save experience:  
starting from now, users with enough permission can decide whether to set a friction to compulsorily ask for confirmation before saving in the Projects of a Company or to alternatively let users save their configuration in a quicker way thanks to the shortcut `Ctrl/Cmd + S`.
