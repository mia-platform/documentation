...
This service will automatically be named _fast-data-delivery-realtime-updater_ and it can be found in the "Microservices" section.
...
:::info
The Real-Time Updater includes more optional parameters that migth be included via Environment Variables. If you need to know more about them, you can [read the documentation of the Real-Time Updater](/fast_data/configuration/realtime_updater.md).
:::

#### Microservice ConfigMaps
...
- **PROJECTIONS_CHANGES_COLLECTION**: it is the name of the collection that includes the Projection Changes generated by the Real-Time Updater, the two must share the same name (we previously suggested _fd-pc-delivery_).
...
:::info
Also the Single View Creator includes more optional parameters that migth be included via Environment Variables. If you need to know more about them, you can [refer to the related documentation section](/fast_data/configuration/single_view_creator.md#environment-variables).
:::
