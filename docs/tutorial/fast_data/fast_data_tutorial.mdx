...
After that, we can try to send message to the Kafka Topic created in the previous steps, and we can see the data stored in the Projection using the APIs available in the [API Documentations](../../development_suite/api-portal/api-documentations)
...
- **KAFKA_BROKERS**: the comma separated list of the kafka brokers that will be used to send the data to the Projections
...
#### ER-Schema
The ER-Schema is the same one we used in the Projection Microservice. We can duplicate the configMap, or we can use a [Shared ConfigMap](../../development_suite/api-console/api-design/services#shared-configmaps).

To simplify, here' the same ER-Schema used in the [Projection ER-Schema](../../tutorial/fast_data/fast_data_low_code_tutorial#er-schema):
```json {4,7,9,13,15} showLineNumbers
{
  "version": "1.0.0",
  "config": {
...
Now we can Commit & Generate our changes, deploy, and try our [Fast Data](../../fast_data/architecture.md) sending data from the Kafka Topic, and see the result in the Single View using the endpoints.
