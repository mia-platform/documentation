...
* The [Backoffice Tutorial Basics](/tutorial/backoffice/basics.mdx).
* The [Backoffice Tutorial Templates](/tutorial/backoffice/templates.mdx).
...
In the second wizard step, provide an `HTML` entrypoint. Use `{{your_address}}/angular` (the same address created in the endpoint step).
...
In the right panel, click on `Edit Property` under the `Menu Items` section. Then click on the `Add Item` button and provide a `Label` and a `Link` for the menu item.
...
In the second wizard step, provide an `HTML` entrypoint. Use `{{your_address}}/react` (the same address created in the endpoint step).
Click on the `Save` button and you will be redirected to the Backoffice page.
...
Now we can save the configuration and deploy. Check the [Console Handbook](/tutorial/console/console_handbook.md) for more information about the deploy command.
...
* The [Backoffice Templates Tutorial](/tutorial/backoffice/templates.mdx) where we explain how to configure other types of pages using the Backoffice Page Composer and an iFrame Page.
* The [Backoffice Basics Tutorial ](/tutorial/backoffice/basics.mdx) where we demonstrate how to create a new Backoffice and how to create a new Backoffice Page using the Backoffice Table Template.
