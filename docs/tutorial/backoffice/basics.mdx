...
# Welcome to the Basics of Mia-Platform Backoffice
...
- A API gateway already created, and optionally an API Portal, follow the [API Portal Tutorial](/tutorial/api_portal/api_portal_tutorial.mdx) if needed.
- A CRUD microservice already created, follow the [CRUD Tutorial](/tutorial/rest_api/quick_rest_api.mdx) if needed.
...
:::warning
If you're unfamiliar with CRUD microservices, consider consulting the [CRUD Tutorial](/tutorial/rest_api/quick_rest_api.mdx).
:::
...
Within the `Pages` tab, located under the `Backoffice` section, you'll find the `Products` page we recently created. Click on the `Compose` button (positioned between the `Edit` and `Delete` buttons) to access the page composer.
...
To modify the form schema in the `Layout` tab on the left panel we can click on the `Main` button and then on the `Form` button.
...
In this case, we want to show only the `name`, `price` and `description` fields in the insert and update form so we can remove all the other fields (creatorId, createdAt, updaterId, updatedAt and \_\_STATE\_\_). About the `_id` field, we can remove it from the insert form because it is an auto-generated field, but we can show it in the update form using the `readOnly` property.
...
To update the tab label, click on the `Tab` button located on the left panel under the `Header` button. Then, on the right panel, click `Edit Property` under the `Tabs` label. In the appearing modal, select the `Edit` option.

![Backoffice Page Composer](img/basics_17-backoffice-page-composer-tab-label.png)

Now, you can modify the tab properties. For our example, change the `Label` input to `All`.
...
With all the configurations in place, save your changes and then deploy. For further details on the deploy command, consult the [Console Handbook](/tutorial/console/console_handbook.md).
...
* In the [Backoffice Templates Tutorial](/tutorial/backoffice/templates.mdx), explore how to configure various page types with the Backoffice Page Composer and an iFrame Page.
* The [Backoffice Microfrontends Tutorial](/tutorial/backoffice/microfrontends.mdx) provides guidance on configuring a Microfrontend using Angular or React, and its integration into the Backoffice.
