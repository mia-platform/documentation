...
In this tutorial we will create a CRUD collection rappresenting books and expose it through REST API. More specifically, we will:
...
1. Fill the input:
   ...
   - **Internal Endpoint**: it is the endpoint exposed by the _CRUD Service_, by default it is the same as the name, but it can be changed. In this case we can use `/books`;
...
At this point, we can modify the DB schema by creating the required properties of your CRUD collection, in this case the one about the books. To do so, you can choose between 2 options:
...
| Name          | Type    | Required | Nullable | Client-side Encryption | Description                    |
| ------------- | ------- | -------- | -------- | ---------------------- | ------------------------------ |
...
:::tip
If you don't know how to do it take a look at the [first tutorial](/tutorial/microservices/hello_world.mdx#expose-the-hello-world-microservice-api)
:::
...
You can now go to the API portal: from your project settings, click on the "Go to Documentation" link of the environment you have deployed in. Under the **books** tag, you can see the list of **endpoints** that will allow us to read, write, edit, and delete the information on your CRUD collection.
