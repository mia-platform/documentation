...
Now, we have to implement the route that calculates the shipping cost for a specific order; let's get back on the IDE and open the `index.js` file:
...
Now, we could push code on the master, deploy and try, but Mia-Platform strongly encourages good use of tests. This allows us to test our microservice without put online and in addition, makes the code easier to evolve over time. After each change, we can run tests to check if all still works.

Let's write a simple test for `/hello` route:

Open `test/index.test.js` file and replace the "Insert your tests.." comment with the following code:
...
Now run tests by the properly NPM script command. In terminal enter:

```zsh
npm run test
```

The tests should be ok and you should see in the terminal the following messages:
...
Let's start writing the route handler: into the `handlers` folder add a file named `getShippingCost.js` as done previously.
...
Let's go back to `index.js` file and add the route `GET /get-shipping-cost` handled by the function write previously:
...
Now let's write a simple test for `GET /get-shipping-cost` route.  
To simplify we will show only one test: the correct calculation of the shipping cost for a new customer. You can find the other tests (no new customer, customer/order doesn't found) in the [tutorial repository]((https://github.com/mia-platform/Node.js-Microservice-Tutorial).
...
Now run tests by the properly NPM script command:
