...
We will add the _API Portal_ and _Swagger Aggregator_ [_Marketplace_ plugins](/runtime_suite/mia-platform-plugins.md) in a project. As a result, we will use the Portal to view and test project endpoints.
...
:::info
If you have already done the [previous tutorial](/tutorial/microservices/hello_world.mdx), you can skip this section and go on using the same project.
:::
...
- a [_Swagger Aggregator_](/runtime_suite/swagger-aggregator/overview.md), that will be contacted by the _API Portal_ to have the entire OpenAPI documentation to be shown. In order to create it, the _Swagger Aggregator_ will retreive the OpenAPI documentation of each service and merge it into a single one.
