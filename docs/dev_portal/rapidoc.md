...
:::
=======
:::caution Due to performance reason **_we highly suggest_** to use this plugin to set up the Dev Portal but in case of `small (<~300kb)` OpenApi specification files it is possible to use the Redoc plugin, we reccomend to read the [Redoc Documentation](../runtime_suite/redoc.md):::
>>>>>>> Stashed changes
