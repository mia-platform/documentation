...
Since the _Fast Data Job Sync_ may need to retrieve Control Plane pipelines from a different database, it's also possible to provide the aforementioned properties
in two different keys, namely `controlPlane` and `dataCatalog`, which represents two different database where the jobs will read or write data.
...
- two database instances:
  ...
  - `dataCatalog.configuration.url`
  - `dataCatalog.configuration.database`
