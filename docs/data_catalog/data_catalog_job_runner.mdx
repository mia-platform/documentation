...
_Agent Dataset Sync_ is a procedure that queries datasources for the data schemas of the resources they own and stores them into the Data Catalog
...
_Data Catalog Sync_ is a procedure that is automatically scheduled after an [_Agent Dataset Sync_](#agent-dataset-sync) job: 
...
_Connection Cleanup_ is a procedure that is launched automatically when a user [deletes a connection from the Data Catalog UI](/data_catalog/frontend/data_catalog_connections.mdx#delete-connection). 
This job removes all the assets of a deleted connection from the Data Catalog.
...
This jobs can be linked to existing [Mia Platform CRUD connections](/data_catalog/frontend/data_catalog_connections.mdx#mia-platform)  where the proper namespace has been defined, so that they can be accessible
from the [Data Lineage section](/data_catalog/frontend/data_lineage.mdx).

#### Trigger Job

To launch the _Fast Data Jobs Sync_ procedure you have to connect to the Job Runner container in your namespace and using tools such as [`grpcurl`](https://github.com/fullstorydev/grpcurl) or [`grpcui`](https://github.com/fullstorydev/grpcui).

Here's an example request that you can make using `kubectl` and `grpcurl`:

```bash
# first, assuming 50051 as `GRPC_PORT`, port-forward the job runner service on local port
kubectl port-forward svc/job-runner 50051
# on another terminal process, call the `Run` service
grpcurl -plaintext -d '{"openLineageFastDataJobSync": {}}' localhost:50051 job_runner_fabric.JobRunner.Run
```
