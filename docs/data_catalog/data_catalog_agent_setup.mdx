...
Once all the data schemas have been imported by _Data Catalog Agent_, it's __required__ to run a separate process to generate all the assets from each data schemas.
...
To perform the synchronization between the collections of datasets and assets, the following cronjob has to be configured. In this [documentation page](/development_suite/api-console/api-design/jobs-cronjob.md)
