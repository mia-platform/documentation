...
- Add the `/open-lineage/*` endpoints to your project's configuration, along with [their permissions](/data_catalog/secure_access.mdx#open-lineage-alle di un confronto con Fede abbiamo concordato che la gestione attuale non è efficace perchè non riusciamo mai a trovarci con le persone necessarie. Quindi, da adesso procederemo in maniera un po' differente, ditemi se vi torna:
 pi).
