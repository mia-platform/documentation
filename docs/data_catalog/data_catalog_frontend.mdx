...
At the top of the page a bredcrumb helps users to easily understand the background structure of Data Catalog navigation patterns. In the example, user has entered the table "TO-BE-CORRECTLY-NAMED" detail page; from the breadcrumb it is noticeable that table "TO-BE-CORRECTLY-NAMED" belongs to System of Records "TO-BE-CORRECTLY-NAMED".  
In case the user wants to easily move from table "TO-BE-CORRECTLY-NAMED" to SoR "TO-BE-CORRECTLY-NAMED" without coming back to the search page to find "TO-BE-CORRECTLY-NAMED" asset, with a simple click on "TO-BE-CORRECTLY-NAMED" user can enter its detail page. Instead, if user wants to choose another table belonging to SoR "TO-BE-CORRECTLY-NAMED", by clicking on the button to the right of the current table name, the list of tables of that SoR is shown and selectable.
...
Once created, the specific custom property can be applicable on each asset that fits the asset type defined for the property. For example, the "TO-BE-CORRECTLY-NAMED" property can be applied on solely Table asset type. Therefore, when accessing any table present on Data Catalog, user can add "TO-BE-CORRECTLY-NAMED" property on it and insert a value compliant with "TO-BE-CORRECTLY-NAMED" custom property type for metadata enrichment on that specific table.
