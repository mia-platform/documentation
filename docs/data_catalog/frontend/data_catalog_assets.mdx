...
* **SoR** (System of Records), corresponding to a Connection configured in [its dedicated section](/data_catalog/frontend/data_catalog_connections.mdx);
...
* **Lineage**, that shows a high-level lineage canvas displaying how a specific SoR is related to other SoRs. For more information about the SoRs-level lineage, visit the [related documentation](/data_catalog/frontend/data_lineage.mdx#system-of-record-level-lineage).

![Details of a SoR page](./../img/sor_details_page.png)

:::info
In case of a **Virtual SoR** detail page, the provider info is not available, because it represents an asset manually created by the user. To know more about the concept of `Virtual Asset`, please give a look at the [related documentation](/data_catalog/frontend/data_lineage.mdx#real-and-virtual-assets)
:::
...
The exported `.csv` file is characterized by a standard structure, involving the following information displayed in the file header:
  ...
  * **Default asset fields**, useful to univocally identify a specific asset (`Asset ID`,`Asset Type`,`System of Record`,`Catalog Name`,`Schema`,`Table`,`Column`)
...
:::info
It is no necessary to have populated all the default asset fields for each row of the exported file. It depends on the asset type and on the type of connection provider of the SoR from which the asset belongs.
:::
