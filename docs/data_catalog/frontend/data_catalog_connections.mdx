...
- __Connections Creation__: the UI provides a creation wizard that let users define a new connection that contributes to Data Catalog assets;
- __Connection Overview__: users can see and edit the parameters of a particular connection. Moreover, users can delete connections that are no longer needed;
- __Connections Management__: users can see a list of connections that populate the Data Catalog application. From this table, users can trigger a [Data Catalog Sync Job](/data_catalog/data_catalog_job_runner.mdx#data-catalog-sync) from the Job Runner, to import connections assets in the Data Catalog.
...
The information that can be provided inside the text fields that are accessible across this section can be of two types:
...
- __Secret Name__: the field will be a text referencing a secret name defined within the _Job Runner service_ (see the paragraph Secrets Name Configuration for more details).
  A dropdown can provide a list of suggestions containing existing secret names retrieved from the service itself. If a secret name that doesn't appear in the list is provided, the service will try to read
...
By clicking on the `Create Connection` button, users are guided through a creation wizard that defines the parameters to connect to a connection.
...
First of all, users needs to choose the provider they want to access from Data Catalog.
...
:::caution
The name of a connection is an information that __can't__ be modified once the connection has been created.
:::
...
- __Connection String__: users needs only to pick a secret name referencing a connection string in the Job Runner Service.
  <div style={{display: 'flex', justifyContent: 'center'}}>
    <div style={{display: 'flex', width: '100%'}}>
...
- __Data Source Name (DSN)__: users can choose between the available DSN configured in the Job Runner Service. 
  ...
  Be aware that username and password are __mandatory__ for connections such as __MSSQL__ or __SAPHANA__.
...
- __Query Mode__: based on this option, the [Data Catalog Sync Job](/data_catalog/data_catalog_job_runner.mdx#data-catalog-sync) will perform the scan query on a different set of views provided by Oracle. Three options are available:
  ...
  - __All__: tables `all_tables`, `all_views` and `all_mviews` will be used. 
    It's __strongly suggested__ to add a comma-separeted list of schemas in the dedicated field to only pick the tables of interest;
...
data models stored on MongoDB through Crud Service.

The required information to setup the connection are:

- the __Url__ where the CRUD Service is located;
- the __Schemas Endpoint__ (usually `/-/schemas`) to retrieve the collections assets;
- the __Health Check Endpoint__ (usually `/-/healthz`) to retrieve the version of the CRUD Service.

Additionally, it's possible to define the project and the environment where the CRUD Service is located in a Mia Platform Project
...
:::info
There's no need to add the header `"accept": "application/x-ndjson"` since it's managed internally by the Job Runner service. 
:::
...
of the [Data Catalog Sync Job](/data_catalog/data_catalog_job_runner.mdx#data-catalog-sync) from the Job Runner. Here you can choose:
...
This section comes already with the values that will used by default in the Job Runner. Once you've clicked on the `Finish button`,
...
After creating the connection, you can reach its overview either through the confirmation modal or the __Connections Panel__.
...
From the menu button on the top-right corner of the page, is possible to click `Delete Connection` 
...
:::caution
This action __will remove any assets that has been imported from that connection in the Data Catalog__, by leveraging the [Connection Cleanub Job](/data_catalog/data_catalog_job_runner.mdx#connection-cleanup) of the Job Runner.
:::

## Connections Management

Once a connection has been defined and it's ready to be imported into the Data Catalog, you can start to synchronize its assets.

In the main page of the Connections Panel, you can clik the `sync button` and schedule a [Data Catalog Sync Job](/data_catalog/data_catalog_job_runner.mdx#data-catalog-sync) from the Job Runner.
Once the action has been executed, the table entry of that connection will be updated by the Job Runner with information about the job status. 
...
Once the _[Data Catalog Sync Job](/data_catalog/data_catalog_job_runner.mdx#data-catalog-sync)_ is completed, a _[Fabric Sync Job](/data_catalog/data_catalog_job_runner.mdx#fabric-sync)_ will be executed automatically to store the assets retrieved from the data source into the Data Catalog.  
...
Only one connection at a time will perform the synchronization: If more than one connecetion is synced at the same time, 
...
Once the synchronization has been completed, a `Sync Completed` message will be returned.
...
If there's an error with your connection parameters (for example, a secret that has not been found by the Job Runner), an error message will be returned by the process.
