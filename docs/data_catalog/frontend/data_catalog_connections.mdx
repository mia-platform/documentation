...
:::info
Since __MongoDB__ is a NoSQL database there's no concept of schema for a collection. However when a [json schema validation](https://www.mongodb.com/docs/manual/core/schema-validation/) is available
...
In particular, the following rules are followed by the agent during retrieval phase to parse the validation object:
...
- when the form of the validation is, at first level, written as:
  ...
  then fields are extracted, since it's possible to determine column assets for every document of the collection;
- if either `anyOf`, `allOf`, or `oneOf` appears at the first level of the validation schema, then the schema won't be parsed.

:::

Other than the connection settings, it's possible to also tune both databases and collections retrieval.
...
* __Strict Discard Default__: the default option, where the agent will try to pull the assets of any non-admin database;
* __Strict__: the agent will try to pull only the assets of the database specified in the connection string;
* __All__: the agent will try to pull the assets of every available database the user can access.
* __Whitelist__: the user can specify a comma-separated list of databases that the agent will try to pull its assets from. 

##### MongoDB Collections Settings

Similarly, the collection retrieval can be tuned with the following options:

* __Strict and Validated__: the default option and the preferred one, since the agent will retrieve only collection having a `validator.$jsonSchema` creation option;
* __Strict__: the agent will try to pull the assets of the collections except the system's collections;
* __All__: the agent will try to pull any collection of the databases.
* __Whitelist__: the user can specify a comma-separated list of collections that the agent will try to pull its assets from. 
...
:::info
Thi settings can be configured for all connections __excepts__ _Mia Platform CRUD_ and _MongoDB_. 
:::
