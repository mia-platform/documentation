...
to perform actions such as [Enrichment](/data_catalog/frontend/data_catalog_assets.mdx) and [Lineage](/data_catalog/frontend/data_lineage.mdx).
...
- __Connections Management__: users can see a list of connections that populate the Data Catalog application. From this table, users can run a Catalog Agent Sync Job of the Job Runner, to import connections assets in the Data Catalog.
...
__Oracle DB__ has also a section which can change the behavior of the underlying queries that the _Agent Job_ will
...
- __Query Mode__: based on this option, the Agent Job will perform the scan query on a different set of views provided by Oracle. Three options are available:
...
of the Agent Job from the Job Runner. Here you can choose:
...
After creating the connection, you can reach its overview of a connection either through the confirmation modal or the Connections Table.
...
:::caution
This action __will remove any assets that has been imported from that connection in the Data Catalog__, by leveraging the Cleanub Job of the Job Runner.
:::
...
In the main page of the Connections Panel, you can clik the `sync button` and schedule an Agent Dataset Sync job from the Job Runner.
Once the action has been executed, the table entry of that connection will be updated by the Job Runner with information about the synchronization process. 


<div style={{display: 'flex', justifyContent: 'center'}}>
  <div style={{display: 'flex', width: '100%'}}>

![Connection Sync Running](./../img/connections_sync_running.png)
