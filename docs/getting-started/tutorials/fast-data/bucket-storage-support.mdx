...
Once the microservices are configured, we can start to overwrite the `application.yaml` configuration of the _Ingestion Storer_ to define the _ingestion topics_. Lets the consider the Registry Projection [from the Fast Data tutorial](/getting-started/tutorials/fast-data/low-code.mdx#registry-projection). Here's the basic configuration that you will have to provide:
