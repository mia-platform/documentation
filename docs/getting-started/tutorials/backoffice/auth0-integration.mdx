...
In this tutorial, we will guide you through the process of securing Mia-Platform Backoffice using Auth0 as your Identity Provider. In particular, we will see how to:
- protect the Mia-Platform Backoffice with the Auth0 login page;
- create, delete and manage both Auth0 users and users' groups from dedicated pages on Mia-Platform Backoffice;
- use the ACLs feature of Mia-Platform Backoffice to display to the user only the subset of pages and components they are authorized to access.
...
- [Configure the Backoffice application](#configure-the-backoffice-application)
...
Before starting this tutorial, you should be familiar with the concepts of the [Mia-Platform Backoffice](/business_suite/backoffice/10_overview.md).
We also recommend to complete the [Backoffice Tutorial Basics](/getting-started/tutorials/backoffice/basics.mdx).

## Create the necessary Microservices

Create the [Secure API Gateway](/runtime_suite_applications/secure-apigateway/overview.md) application:
...
You can follow this page: [API Gateway Configuration](/runtime_suite_applications/secure-apigateway/configuration.md#api-gateway).
...
You can follow this page: [Authorization Service Configuration](/runtime_suite_applications/secure-apigateway/configuration.md#authorization-service).
...
```
https://your-backoffice-project.mia-platform.eu/web-login
```
...
* `AUTH0_CALLBACK_URL`: the url already defined during Auth0 configuration (i.e. https://your-backoffice-project.mia-platform.eu/web-login/callback)
...
## Configure the Backoffice application
Now that you have configured all the microservices you need to follow these last steps to configure the Backoffice to enable the creation of Auth0 users.
...
### Add the pages to the Backoffice

Then you can add the newly created pages to the Backoffice menu:
...
With the **ACL Expressions** you've just set, users outside `groups.admin` will see these pages in the left-side menu of the Backoffice, but will receive a 404 status code page. 
...
![Backoffice Homepage](img/auth0_07-homepage.png)
...
![Backoffice Page Composer](img/auth0_08-create-config.png)
...
Congratulations! You have successfully created your first user using the backoffice.
If you try to access the Backoffice with this new user you won't see the users management pages that are protected with the ACLs created in the previous steps.
