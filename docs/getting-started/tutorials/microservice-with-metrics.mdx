...
These metrics will then be available in [Grafana](/docs/paas/grafana.md) to use in personalized dashboards based on user-defined queries.

## Pre-Requisites

Before starting, we assume that you have already created the `shipping-cost` microservice from the [Create a custom Microservice tutorial](create-a-custom-microservice.mdx).
...
More-over, the project where the service has been created must have a monitoring system such as Promethoeus configured from the [Microservice Monitoring Section](/docs/development_suite/api-console/api-design/microservice-monitoring.md):
...
<details><summary>Test the behaviour of your metrics:</summary>
<p>
...
![Crea New Metrics Dashboard](img/metrics-new-dashboard.png)
...
:::tip
This dashboard can also be embedded into the Console using the [Dashboard Section](docs/development_suite/monitoring/dashboard).
:::