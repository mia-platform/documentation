...
|`headers`| - |{ [x: string]: string; }| - | - | - | - |
|`showPdfInBrowser`|`show-pdf-in-browser`|boolean| - | - |false|allows to display PDFs inside a visualizer instead of downloading them. If true, a component like `bk-pdf-viewer` should be included in the configuration.|
...
| event | action | emits | on error |
|-------|--------|-------|----------|
|[http-success](../Events/Events#http-success)|terminate a successful registered transaction| - | - |
...
If export must be performed on a [CRUD-service](../../../runtime_suite/crud-service/overview_and_usage) resource, an export route is available at `<collection-name>/export` and supports
