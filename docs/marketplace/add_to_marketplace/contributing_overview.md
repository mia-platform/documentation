...
The values of the `defaultLabels`, `defaultAnnotations`, and `defaultEnvironmentVariables` fields can contain placeholders that will be replaced with the actual values when the service is created by a Console user.

Here is an exhaustive list of the placeholders that can be used:

- `%MICROSERVICE_NAME%`: the name of the created microservice
- `%PROJECT_ID%`: the human-readable ID of the project. It is a dash-separated string that is generated by the Console when a project is created, based on the project name.
...
As an example, given a Project with the id `my-project` and a Company with the ID `my-company`, a Console user creates a Microservice from such plugin, naming the Microservice `my-ms`.
