...
Envoy offers a valid alternative to our [API Gateway based on NGINX](/runtime-components/plugins/api-gateway/10_overview.md). Despite its raw configuration may not be straightforward to write and understand, it provides many advantages:
...
- **Extensions:** Envoy offers a vast variety of [filters](/runtime-components/plugins/envoy-api-gateway/filters.md), that can be added to improve and customize the default configuration according to the project's needs.
...
So, to configure [ServiceMonitor](/products/console/api-console/api-design/microservice-monitoring.md#configure-servicemonitor) in Console, you should set:
