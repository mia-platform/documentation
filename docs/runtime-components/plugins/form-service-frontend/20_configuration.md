...
Configure the **Form Service Backend** following [this guide](/runtime-components/plugins/form-service-backend/20_configuration.md) before using the **Form Service Frontend**.
...
Knowing the frontend endpoints described in the [overview](/runtime-components/plugins/form-service-frontend/10_overview.md#how-it-works) can be useful during the configuration process.
...
In addition, the `form_schemas` CRUD and *submit urls* CRUDs must be added as [CMS pages](/products/microfrontend-composer/previous-tools/cms/config_cms.md#how-to-create-a-page) in order to access the configured form templates and submitted forms data.

Finally, to edit a form template, visualize the resulting form and access the submitted forms data, [links](/products/microfrontend-composer/previous-tools/cms/conf_cms.md#navigation-between-collection-with-link) to the previously integrated frontends must me added.
...
Further details about the API Gateway and how can be further configured based on your needs can be found [here](/products/console/api-console/advanced-section/api-gateway/how-to.md)
