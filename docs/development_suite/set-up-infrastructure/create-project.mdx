...
whose lifecycle can then be entirely managed and developed through Console areas.
...
- **Git Provider**: The Git Provider will be used by the Console to correctly retrieve the third party service configurations needed by your projects.
...
- **Template**: Repository with pre-filled configurations. Each company could have one or more template.
  ...
  In short, it constitutes a base on which to create your project. [Here](/development_suite/company/project-templates.md) you can find a guide on how to handle project templates.

:::tip
If you are using the PaaS Mia-Platform Console, two main templates are available: one with all core services and the CMS, the other with only the core services. More information about how to use them at [Setup PaaS Templates](/development_suite/set-up-infrastructure/create-project.mdx#setup-paas-templates).
:::
...
2. **Repository**: in this phase, you need to choose the project template, and an overview is also presented on the default Git Provider configured for the company, which will be inherited by the project:
...
3. ** Environments**: in this phase, information about the environments that will be created for the project is shown:
...
Once your project has been created, you will be redirected on the Project Overview area of your new project, where you can see your environments and a list of environment variables:
...
:::info
It is possible, through CMS, to go and change the providers (Git Provider, Secret Manager, CI/CD Tool) set for the project or its environments at a later time.
To do so, please refer to [these](/development_suite/set-up-infrastructure/configure-provider.mdx#associate-providers-to-projects) guidelines.
:::
