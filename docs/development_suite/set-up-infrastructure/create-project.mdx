...
During project creation, you will be able to configure a project in its entirety.
Once created, the project lifecycle will be then entirely managed and developed through the areas of the Console.

## Requirements

The first section of this guide informs you about the requirements needed to create a project.
...
- **Git Provider**: the Git Provider is the resource that will be used by the Console to correctly retrieve the third-party service configurations needed by your projects.
  You can create a Git Provider for your company by following the procedure described [here](/development_suite/set-up-infrastructure/configure-provider.mdx).

- **Template**: a template is a repository with pre-filled configurations. Each company can have one or more templates.
  The template allows you to pre-fill resources in your project and start similar projects with the same configuration.
...
Once you have both the provider, the company, and the template correctly configured, you will be able to create a new project.
...
2. **Repository**: in this phase, you will be asked to choose the repository basic configuration and the project template. You also will be asked to select a Git Provider, starting from the default Git Provider configured for the company:

  ![create-project-2](img/create-project-2.png)

   * **Git provider**: one of the supported Git Provider types. Check out the complete list of [supported providers](/development_suite/set-up-infrastructure/configure-provider.mdx);
...
Here you will be able to see the [project basic settings](/development_suite/set-up-infrastructure/project-settings.md), the [project environments](/development_suite/set-up-infrastructure/runtime-environments.md) and a list of [environment variables](/development_suite/set-up-infrastructure/env-var-intro.md):

### Environment Variables

Once you create a new project, your project will already include variables. Some variables will be already pre-filled from the project or the company default settings, such as `_CMS_IMAGE_NAME_`, `_LOG_LEVEL_`, and `_CRUD_LIMIT_CONSTRAINT_ENABLED_`, while others will not be pre-filled (such as `_NOTIFICATIONS_COLLECTION_ROUTES_` and `_NOTIFICATIONS_SERVICE_PATH_`).
