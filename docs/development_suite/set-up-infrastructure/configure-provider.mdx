...
* **type** (*required*): one of the supported provider types (`gitlab` or `vault`).
...
* **providerId**: the unique identifier of the provider (obtainable from the providers' CRUD collection);
* **type** (*required*): one of the supported provider types (`gitlab-ci`, `azure-pipelines`, or `jenkins`).
