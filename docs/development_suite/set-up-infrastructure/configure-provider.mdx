...
More specifically, a user can:

* Add a new connection to a provider;
* Edit the connection to a provider;
* Set a default provider for a Company.

Moreover, only for Git Providers, a user can:

* Edit the accessibility to the Git Provider and its related resources;
...
Please note that, in order to select the "Marketplace" functionality for a Git Provider, "Secret Manager" and "CI/CD tool" capabilities will be not available for that specific Git Provider. 
...
Only if the provider you have chosen is a Git Provider and you have selected the "Marketplace" functionality, in the Step 2 you will also see a toggle – enabled by default – to manage the access to the resources connected to that Git Provider. When enabled, this toggle allows other Companies to access those resources. 
...
If you have a Git Provider with selected the "Marketplace" functionality, you are going to make available to the other Companies all the resources connected to that specific Git Provider. This action is reversible: you can manage the accessibility to that Git Provider at any time, by simply switching off the accessibility toggle. 
