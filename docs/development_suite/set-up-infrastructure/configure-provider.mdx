...
Only if the provider you have chosen is a Git Provider, and you have selected the "Marketplace" functonality, in the Step 2 you will also see a toggle – enabled by default – to manage the access to the resources connected to that Git Provider. When enabled, this toggle allows other Companies to access those resources. 
