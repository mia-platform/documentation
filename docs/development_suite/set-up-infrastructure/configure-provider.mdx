...
If you are connecting a new Provider for Marketplace, you will also see an additional toggle, which allows you to manage the accessibility to the Provider.
...
For more information about how to share a Marketplace Resource, please refer to the [dedicated documentation page](/docs/marketplace/add_to_marketplace/marketplace_items_accessibility).


:::note
Marketplace related Providers are distinguished by the label "**for Marketplace**".
:::


![Provider overview Marketplace](img/providers-overview-marketplace.png)

The process of connecting a Provider for Marketplace is the same as the one described in the [Connect a Provider](#connect-a-provider) section, with only small differences in the first and second steps.
Here below you can find the whole process described. TODO: check this
