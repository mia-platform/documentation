...
- **providerId (string)**: it will be used to identify the provider;
...
- **proxy (object)**: an object that configures the proxy agent for the provider calls, it contains:
  - **url (string)**: the proxy url, this prop is required if you want to configure a proxy for your provider.
  - **username (string)**: the username used to authenticate with basic auth during the proxy connect request.
  - **password (string)**: the password used to authenticate with basic auth during the proxy connect request, this property is safely encrypted.
- **Base64CA (string)**: certificate of the trusted Certification Authority
...
For more information about MongoDB encryption, take a look at the dedicated [documentation section](/runtime_suite/crud-service/encryption_configuration.md).
:::
