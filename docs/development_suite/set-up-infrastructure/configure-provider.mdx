...
Regarding the **Git Provider** capability it can be set to one of the following two functionalities:

* **Project**: this provider is configured to operate on your Mia-Platform projects
* **Marketplace**: this provider is configured to operate on your own Marketplace items.  
When a provider is set with this functionality, it can be configured to be accessible also to other Companies.
...
Moreover, only for Git Providers, authorized users can:

* Choose the Git Provider usage functionality between "Project" and "Marketplace".  
* Edit the accessibility to the Git Provider for Marketplace and its related resources;
...
If you select a "Git Provider", you can also select its purpose of use, by choosing between "Project" (by default) or "Marketplace" functionalities.  
If you preserve the default choice of "Project" functionality, the Git Provider will be specifically used for your Projects workspaces. By selecting "Marketplace", instead, you will be able to make any resources related to that Git Provider available to other Companies as well.  
Please note that, in order to select the "Marketplace" functionality for a Git Provider, "Secret Manager" and "CI/CD tool" capabilities will not be available for that specific Git Provider. 

:::info
If the provider offers only one capability, clicking on it will lead directly to step 2 and the capability will be selected by default. However, if the provider is a Git Provider, you will equally remain in step 1 since you will still be able to choose the functionality of usage for that Git Provider, which can be "Project" (by default) or "Marketplace".
:::
...
#### Provider Details of a Git Provider for Marketplace

Only if the provider you have chosen is a Git Provider and you have selected the "Marketplace" functionality, in the Step 2 you will also see a toggle – enabled by default – to manage the access to the Git Provider and the resources connected to it. When enabled, this toggle allows other Companies to access those resources. 
...
#### Edit a Git Provider for Marketplace accessibility

If you have a Git Provider with the "Marketplace" functionality selected, you are going to make available to the other Companies all the resources connected to that specific Git Provider. This action is reversible: you can manage the accessibility to that Git Provider at any time, by simply switching off the accessibility toggle. 
...
## Associate Git Providers for Marketplace

When adding a new Git Provider connection, you can change the utilization scope of that specific Git Provider, by leveraging its "Marketplace" functionality. This represents an alternative of usage which is available alongside the default "Project" functionality.  
Once the "Marketplace" functionality for a Git Provider is selected, it allows the accessibility of any resources associated with that Git Provider (e.g., Marketplace items and/or Project templates) to other Companies.
