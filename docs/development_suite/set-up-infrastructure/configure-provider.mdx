...
## Provider APIs
For backward compatibility reasons, we report in this section the APIs used to create and modify providers, along with examples of request bodies:
...
The fields `capabilities` and `expirationDate` (within `credentials`) have been introduced starting from the version 10.8 of the Console.
:::
