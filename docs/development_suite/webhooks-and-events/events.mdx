...
This event will be triggered when a microservice is generated successfully from a marketplace template or example.

**Note**: the event is triggered as soon as the service's repository is successfully created, hence there is no guarantee that the service will be found into the project configuration.
