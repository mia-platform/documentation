...
- **With limits, with or no requests**: Recommended for most cases, if requests isn't set it will automatically be set by Kubernetes as the same as limits. In this case you are sure that the pods are not stealing each other's resources, but you can have cases where excess CPU is not exploited, going to CPU throttling in case of high number of requests, even though it would not be necessary. The QoS for this case can be:
  - `Guaranteed` QoS, if requests equals limits and both explicitly defined in configuration, meaning that the pod is guaranteed to receive the required resources. If such resources cannot be reserved, it will not scheduled. This pods will be the last to face eviction;
  - `Burstable` QoS in case only the limits it is explicitly defined.
...
- **CPU**: how much CPU is reserved for the container, in Mia-Platform Console it is definded in _millicores (m)_, where 1000 millicores equals 1 core (For example, If the container needs only ¼ of a core, you will put the value of "250m").  
