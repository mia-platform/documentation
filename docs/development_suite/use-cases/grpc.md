...
:::tip
Make sure that the _Use downstream protocol_ flag (in the [Endpoint settings _Advanced_ tab](/development_suite/api-console/api-design/endpoints.md#manage-advanced-endpoint-parameters)) is flagged, this ensures that the `HTTP` protocol version is inferred from the incoming request, allowing the protocol HTTP/2 to be used effectively.
:::
