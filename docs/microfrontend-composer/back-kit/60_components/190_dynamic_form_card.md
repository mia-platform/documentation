...
<<<<<<< HEAD:docs/microfrontend-composer/back-kit/60_components/190_dynamic_form_card.md
[img-bk-form-card]: img/bk-form-card.png
=======

>>>>>>> main:docs/business_suite/backoffice/60_components/190_dynamic_form_card.md
...
An [example](#example-working-with-views) below shows how to configure a Dynamic Form Card that handles writable views.
...
The following example shows a configuration of the Dynamic Form Card designed to interact with [writable views]:
...
- the default `submitButton` is overridden by a custom button that performs a POST with the whole body of the form to "/orders-view/" endpoint.
