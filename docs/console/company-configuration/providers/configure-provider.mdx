...
If in [Step 1](#step-1-provider-services) the **Git Provider** capability has been selected, an additional **Repository path template** field will appear in this step, this field controls where project repositories, created using this provider, will be located. It can also contain a special interpolation variable `{{projectId}}` that will be replaced with the actual project identifier during the [project creation](/console/project-configuration/create-a-project.mdx) phase.
