...
However, in some very specific scenarios, you may want to generate these files differently: in this case, you have to use an **Orchestrator Generator**.  
...
## When should I use the Orchestrator Generator extension?

Orchestrator Generator extension is the go-to when you need to generate orchestrator manifest files tailored to your specific needs. 

Here are some use cases for which you might consider using the extension.

1. **Deploying on Non-Supported Orchestrators**  
    Since Mia-Platform Console currently only generates manifest files to deploy your services on Kubernetes, this extension allows you to bridge the gap and deploy your applications on your preferred orchestrator such as Docker Swarm, Apache Mesos, Amazon ECS, Azure ACS, Google GKE or any other.

2. **Utilizing alternative k8s Package Managers**  
    Even if you're using Kubernetes, the Orchestrator Generator extension might be valuable if you prefer a k8s package manager other than the default one.  
    For instance you may prefer using Helm for managing your deployments, with the extension you can fill this  

3. **Integrating with Existing Orchestrator Workflows**
    If you already have established workflows and tools for managing deployments on your chosen orchestrator, the extension can act as a bridge.  
    By providing your Project configuration, you can leverage your existing workflows and integrate them with the Console, streamlining your overall deployment process.
