...
2. **Repository**: in this phase, you will be asked to choose the basic configurations of the repository associated with your Project and the template to start the Project from. You also will be asked to select a Git Provider, starting from the default one configured for the company:
   ...
   * **Git repo path**: the path, calculated from Project name and company, where the configuration will be saved on the Git Provider. It specifies the location of your Project on the Git Provider website;
    ...
    <TabItem value="gitlab" label="GitLab" default>
      ...
      then the Git repo path must be **`mia-platform/projects`**.
    ...
    <TabItem value="github" label="GitHub">
      ...
      then the Git repo path must be **`mia-platform`**

    </TabItem>

    <TabItem value="azure-devops" label="Azure DevOps">

      It must be in the form `<ORGANIZATION_NAME>/<PROJECT_NAME>/_git/<REPO_NAME>` where:
      - `<ORGANIZATION_NAME>` is the name of your Azure DevOps organization.
      It must be already created on your Azure Devops account and the service account used by the Console needs to have access to it.
      - `<PROJECT_NAME>` is the name of the Azure DevOps project where the Console will create the `configurations` repo.
      If it is not already present in the Azure DevOps organization it will be created by the Console.
      However, in this case the service account used by the Console need to have the permission to create new projects in the organization.
      - `REPO_NAME` is the name where you want to store the configurations of your project, usually named `configurations`
      
      For example, if your organization is reachable at the URL **`https://dev.azure.com/mia-platform-devops`**, <br/>
      then you can use **`mia-platform-devops/Test/_git/configurations`** as Git repo path.  
      In this case, if `mia-platform-devops` organization already contains a `Test` project, then a new `configuration` repo will be created inside that project;
      if the `Test` project does not exists it will be created along with the `configuration` repository. 
