...
To create the Service Principal you just need to:
...
* **client credentials**: with Client Credentials approach you will get a Bearer Token calling the Azure `/oauth` API using a `cliendId` and a `clientSecret`;
...
First of all, you need to generate a certificates pair.
...
* upload the **public key**, in one of the supported formats;
* _(recommended)_ give the certificate a description.

:::warning
Remember to store the "`private` - `public`" certificates pair **in a secure store**.
:::
...
:::info
The secret key can be interpolated. This can be useful if you use one AKV for more runtime environments and, consequently, the name of a secret can be different in different environments.
:::
