...
You can follow [this guide](/docs/tutorial/rest_api/quick_rest_api#create-crud-service) to create the service.
...
:::warning
- Currently, if the database is already populated, all records in all collections must be decorated with the `__STATE__` field set to `'PUBLIC'` in order to access the data. All records that do not have the `__STATE__` field are not returned by the crud-service.
