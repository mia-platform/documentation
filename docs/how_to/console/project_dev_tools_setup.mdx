...
:::warning
- Currently, if the database is already populated, all records in all collections must be decorated with the `_STATE_` field set to `'PUBLIC'` in order to access the data. All records that do not have the `_STATE_` field are not returned by the crud-service.
