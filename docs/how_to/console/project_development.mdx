...
The Mia-Platform Console offers an overview for all microservices created in which is visible a section [Container ports](/development_suite/api-console/api-design/microservice-container-ports.md) where you can view and manage container ports.
...
To do so `service-A` has to make a request equivalent to the following cURL:

```curl
curl --request GET 'http://service-B:80/my-api'
```
