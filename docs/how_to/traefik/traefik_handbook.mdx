...
The secret needs to be created through one of the [supported environments variable provider](https://docs.mia-platform.eu/docs/development_suite/set-up-infrastructure/env-var).
In this hands-on we will refer to the secret as `BASIC_AUTH_USERS`.

#### Generate the secret with mlp
We also need to edit the [mlp](https://docs.mia-platform.eu/docs/runtime_suite_tools/mlp/generate) configuration to interpolate the secret created in the previous section.
...
At this step you only need to [deploy](https://docs.mia-platform.eu/docs/development_suite/deploy/overview#introduction) and you will have the basic auth at the basepath `my-hostname.eu`.
