...
If the Notification Manager receives an unknown event, i.e. an event for which there is no handler registered to process it, an error will be returned if you sent an HTTP request to `POST /notification-events/` and the error details are saved into the [events CRUD](20_configuration.md#events-crud).
...
You can find more details on how to register a custom handler in the [*Configuration* section](20_configuration.md#custom-event-handlers).
...
Each user in your [users CRUD](20_configuration.md#users-crud) may belong to one or more groups and clusters and have one or more roles, saved in different properties of the CRUD, for example `groups`, `clusters` and `roles`; the properties can be named however you want, as explained in the [dedicated section](20_configuration.md#users-crud).
...
To standardize the messages you send to your users, you can create and save a template on [the CRUD](./configuration.md#templates-crud) messages templates.

These are pre-defined messages for the various channels that can be referenced using:

- the `templateId` parameter in the body of the [`POST /send`](./30_usage.md#post-send) or [`POST /saga/send`](30_usage.md#post-sagasend);
...
When using **whatsapp** instant messages, to standardize the messages you send to your users, you can manually create messages template from the [WhatsApp channel management section on Kaleyra platform](https://developers.kaleyra.io/docs/message-templates).
...
Once a template has been approved, you can use the template saving its name on [an appropriate CRUD](./configuration.md#templates-crud) messages templates, and referencing the template using:

- the `templateId` parameter in the body of the [`POST /send`](./30_usage.md#post-send);
- the `templateName` field in the [notification settings][crud-notification-settings].
...
:::warning
WhatsApp supports templates localization, meaning that every template can have messages in different languages. Since the Notification Manager does not currently allow clients to specify a language in the request, we only support the `en` language code. If you need to handle localization, you have to create different templates on [Whatsapp channel management section on Kaleyra platform](https://developers.kaleyra.io/docs/message-templates). This may change in the future.
:::
