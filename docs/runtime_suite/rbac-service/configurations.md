...
:::caution
The `resourcePermissionsMap` will be injected in the user input data only on APIs specifying the `option.enableResourcePermissionsMapOptimization` feature flag to `true` in the `x-permission` configuration.
:::
