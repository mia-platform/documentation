...
In case of a multiple-bucket service, the parameter `scope` should be specified in the form-data section. All the considerations from the previous section still applies.
...
Examples of multipart request to upload a single file with multiple-bucket support:
...
Consider the following simplified example of a HTTP multipart request with multiple-bucket support:
...
In case of a multiple-bucket service, the parameter `scope` should be specified in the form-data section. All the previous considerations still apply.
