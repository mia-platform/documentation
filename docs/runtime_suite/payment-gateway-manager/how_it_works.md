...
The following example is base on the credit card payment method, but it can be applied to any payment.
...
:::warning
At the moment the `satispay` provider doesn't support refunds
:::
...
::: warning This endpoint is only available for `satispay` provider at the moment. :::
