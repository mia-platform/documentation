...
### Notes
Remember, the permissions for a role will always consist of the leaf node values, but the [permissionsManagementEndpoint](#permission-endpoint) must provide the entire tree structure, including internal nodes, for the modal to work properly.

When adding a new permission, such as "write" under a new category "Documents" the [permissionsManagementEndpoint](#permission-endpoint) must return:

- An internal node for the "Documents" category with the value "documents".
- A leaf node for the "write" permission with the value "documents.write".
