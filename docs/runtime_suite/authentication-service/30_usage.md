...
- **appId**: the `APP_ID` key from the [configurations file](/runtime_suite/authentication-service/20_configuration.mdx#configurations-file)
...
Furthermore, `permissions` field is set only in case any permission is added to the permissions array on the document containing user metadata within the [User collection](/runtime_suite/authentication-service/20_configuration.mdx#users-collection).
...
The `/userinfo` endpoint can be used by the [Authorization Service](/runtime_suite/authorization-service/10_overview.md) to determine whether the requested resource can be accessed by the current user, based on the contents of the `groups` array.

You need to set the following variables in the [configuration](/runtime_suite/authorization-service/20_configuration.md) with these values:
...
If you use [*mlp*](/runtime_suite_tools/mlp/10_overview.md), the Mia-Platform deploy CLI tool, to release custom secrets add these lines to the `mlp.yaml` file in your project:
