...
Starting from version _v0.4.3_, the Mongezium configuration file supports the `connections` property, which can be used to create
references of Kafka connections and MongoDB connections and reuse them, respectively, inside the `stream` property
and the `persistence` property.
This trait is similar to other Fast Data services like [Stream Processor](/products/fast_data/fast_data_engine_v2/stream_processor/10_Overview.md),
...
and it will help you to simplify the structure of the configuration file, avoiding to write multiple times Kafka or MongoDB connections.
