...
- the type of operation it was applied onto the record, which ca be:
  ...
  - `READ` &rarr; the record is extracted from the static data source during
    a snapshot procedure
  - `CREATE` &rarr; the record was just inserted in the static data source
  - `UPDATE` &rarr; the record was updated on the static data source
  - `DELETE` &rarr; the record was deleted from the static data source
...
<SchemaViewer schema={EventPayloadSchema} />
