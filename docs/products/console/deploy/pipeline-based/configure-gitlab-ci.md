...
:::info
If your project includes [Helm chart configurations](/products/console/api-console/api-design/charts.md), your pipeline template must use the **`unified-deploy-job.yml`** template and require **MLP version ≥ 2.7.0-helm** to correctly resolve and **deploy with Kustomize** the chart files generated by the Console. Jobs must also extend **`.deploy_kustomize`** instead of the generic `.deploy_job`.
:::
