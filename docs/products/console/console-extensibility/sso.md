...
- If your extension backend also validates the token's `iss` (issuer) claim, update it: the issuer is no longer the Console base URL but the `extensibility` Keycloak realm (see [Validate Access Token](#validate-access-token)).
:::
...
- **iss**: the *issuer* of the token. This is the `extensibility` Keycloak realm used by the Console instance to authenticate extensions
