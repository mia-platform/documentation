...
To deploy projects that include chart configurations, the pipeline must use a **Kustomize-based deploy** strategy. Make sure your pipeline template references the **`unified-deploy-job.yml`** template and jobs extend **`.deploy_kustomize`**, and that **MLP version ≥ 2.7.0** is used.
