...
In [this section](/products/console/project-configuration/auth-flow/authorization-flow) we described how the authorization flow works for a single project. In few words, in order to activate the authorization flow for a project you need to install the **Authorization Service**.  
