...
In the Advanced section of the Design Area you can configure the [Swagger Aggregator](/runtime-components/plugins/swagger-aggregator/10_overview.md).
...
:::note
This configuration affects the API documentation available in the [Documentation Portal](/products/console/project-configuration/documentation-portal.md).
:::

:::caution
The custom configuration can conflict with [endpoints visibility](/products/console/api-console/api-design/endpoints.md#manage-the-visibility-of-your-endpoints) and [API documentation path](/products/console/api-console/api-design/services.md#microservice) set from the Console.
:::
...
:::info
Remember that all the following properties (with the exceptions of `servicesUrlsBefore` and `servicesUrlsAfter`) can also be set through the configuration file, as described [here](/runtime-components/plugins/swagger-aggregator/10_overview.md).
:::
...
Use first level `prefix` property to add to all routes a prefix. This feature may be useful when using multiple [API Gateways](/runtime-components/plugins/api-gateway/10_overview.md) or when having a custom reverse proxy before your project.
