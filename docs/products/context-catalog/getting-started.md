...
- **Configuration → Relationship Types** to see the connection types you can use to wire items together (see [Relationships](./basic-concepts/60_relationships.md)).
...
- **OpenAPI v3.1 schema** — the validation schema for the `spec` of the items (see [Item Types](./basic-concepts/20_item-types.md#the-validation-schema)).
- **Selectable fields** — the `spec` fields you want to filter on later (see [Selectable Fields](./basic-concepts/20_item-types.md#selectable-fields)).
...
Open the detail page of an item, jump to the **Relationships** tab, and link it to another item using one of the [built-in relationship types](./basic-concepts/60_relationships.md#built-in-relationship-types) (e.g. `ownership`, `part-of`, `depends-on`) or one you have defined yourself. From now on, you can navigate that connection from either endpoint, both in the table view and in the graph view.
...
- [Items](./basic-concepts/10_items.md) and [Item Types](./basic-concepts/20_item-types.md) — the data model.
- [Relationships](./basic-concepts/60_relationships.md) — the graph layer.
