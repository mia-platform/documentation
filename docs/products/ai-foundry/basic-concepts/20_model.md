...
| `supportsTools` | No | Set to `true` if the model supports tool/function calling. Used by the AI Playground to determine which agent features are available. Defaults to `false`. |
| `supportsVision` | No | Set to `true` if the model accepts image inputs. Defaults to `false`. |
| `contextWindow` | No | Maximum context window size in tokens. Informational; surfaced in the model detail view. |
