...
- in our example architecture, you can configure the _Stream Processor_ services that ingest messages to the _topic.merge_ topic to evenly divide the partitions to use by configuring them with the special [partition settings configuration](/product/fast_data_v2/stream_processor/???) to dedicate specific partitions to the backup flow and specific partitions to the regular flow, to have a clearer separation of the two flows and better regulate the speed of the reingestion of the backup messages with the speed of the ingestion in the regular flow
